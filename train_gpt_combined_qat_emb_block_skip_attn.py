...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "parallel_post,parallel_resid,attn_gate_proj,block_skip_attn,attn_res_query",
    ).split(",")
    if pattern
)
...
class BlockSkipAttention(nn.Module):
    """Block-attention residual layer adapted for decoder U-Net skip mixing.

    Same core computation as BlockAttentionResidual in
    train_gpt_block_attn_wandb_tracking.py: a learned pseudo-query attends over
    RMS-normalized source/block representations and returns the softmax-weighted
    value sum. In this combined model it is used only at decoder skip-merge
    points, with sources [current decoder state, matching encoder skip].
    """
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 with_block_skip_attn: bool = False):
        super().__init__()
        ...
        # Constructed only for decoder blocks that receive a U-Net skip.
        self.block_skip_attn0 = BlockSkipAttention(dim) if with_block_skip_attn else None
        self.block_skip_attn1 = BlockSkipAttention(dim) if with_block_skip_attn else None
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                with_block_skip_attn=(
                    self.num_encoder_layers <= i < self.num_encoder_layers + self.num_skip_weights
                ),
            )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            # Apply U-Net skip connection through block-skip attention.
            # Sources are [current decoder state, matching encoder skip].
            if skips0:
                sw = self.skip_weights[i].to(dtype=lane0.dtype)[None, None, :]
                s0 = sw * skips0.pop()
                s1 = sw * skips1.pop()
                block = self.blocks[layer_idx]
                if block.block_skip_attn0 is None or block.block_skip_attn1 is None:
                    raise RuntimeError("decoder block is missing block_skip_attn modules")
                if layer_idx >= self.parallel_start_layer:
                    lane0 = block.block_skip_attn0([lane0], s0, layer_idx, 0)
                    lane1 = block.block_skip_attn1([lane1], s1, layer_idx, 1)
                else:
                    x = block.block_skip_attn0([x], s0, layer_idx, 0)  # s0 == s1 for sequential layers
                    lane0 = x
                    lane1 = x
