"""
COMBINED: PR × DR × Attention Gate (architecture)  +  GPTQ + LQER (compression).

Merges the architecture from train_gpt_dr_pr_gates.py (parallel residuals +
depth recurrence + per-head attention output gate, project headline 1.2927
± 0.0007 at 9 layers / INT8 quant) with the GPTQ+LQER post-training
quantization from train_gpt_gptq_lqer.py (asymmetric group-wise int6 for
attn/MLP matrices, int7 for embeddings, LQER rank-4 correction on top-k
most-sensitive layers).

The motivation: GPTQ+LQER compresses substantially better than INT8 at
similar quality, freeing budget under the 16 MB cap. Use the freed budget
to add a 10th or 11th transformer layer.

Quantization is switchable via USE_GPTQ_LQER (default 1). The model
architecture is identical to train_gpt_dr_pr_gates.py — all small/control
tensors (attn_gate_proj weights, parallel_post/resid_lambdas, skip_weights,
etc.) are auto-passthrough under gptq_lqer's INT8_KEEP_FLOAT_MAX_NUMEL=65536
threshold, so no monkey-patching of gptq_lqer's CONTROL_TENSOR_NAME_PATTERNS
is required.

Headline 9-layer config (the project winner, gives 1.2927 ± 0.0007 at 3 seeds):
  NUM_LAYERS=9, PSL=4, RECUR_LAYERS="2,3,4,5", RECUR_TARGET=both,
  GATE_ATTN_OUT=1, GATE_ATTN_SRC=proj, GATE_WIDTH=12.

For NUM_LAYERS=10 or 11, psl and recur_layers default to the same values
(extra layers extend the parallel-no-recurrence region) but are env-var
overridable if you want to re-explore those axes at higher depth.

----------- ORIGINAL ABL8 DOCSTRING BELOW (for context) -----------

Ablation 8: Attention Output Gate × Parallel Residuals × Depth Recurrence (triple stack).

Composes the THREE multi-seed-validated standalone winners from prior ablations:
  - Parallel residuals (abl4):  psl=4, sym init                  → 1.3057 ± 0.0016
  - Depth recurrence   (abl5f): recur=[2,3,4,5] target=both      → 1.3022 ± 0.0029
  - Attention gate     (abl7d): src=proj, width=8                → 1.3042 ± 0.0004 (w=8 winner)
                                                                  → 1.3058 ± 0.0008 (w=12 leaderboard)

The headline test: do three mechanistically-independent mechanisms compose nearly
linearly the way PR × DR did (92.7% efficient, abl6 → 1.2987 ± 0.0028)?

Predictions (assuming PR×DR's 92.7% efficiency carries over to the triple):
  pure additive   : 1.3101 + (-0.0114) + (-0.0059) = 1.2928 (w=8)
  92.7% efficient :                                 ~1.2940 (w=8)
  PR×DR-only      :                                  1.2987 (no extra gate gain)

Per-layer behaviour:
  i <  parallel_start_layer  → sequential block (Block.forward applies resid_mix);
                                if i in recur_layers: extra Block.recur_sub passes
                                  (target=attn|mlp|both; resid_mix NOT re-applied)
  i >= parallel_start_layer  → parallel block (two lanes, learned routing matrix);
                                if i in recur_layers: extra _parallel_recur_block passes
                                  (re-runs attn+mlp through the routing matrix;
                                   resid_mix gate NOT re-applied)

The attention gate sits *inside* every CausalSelfAttention block, regardless of
whether that block runs in sequential or parallel mode. It is applied to the
SDPA output before the proj, with the same math as abl7:
  g = 2 * sigmoid(W_gate @ src[..., :gate_width])  [B, T, H], W_gate zero-init.

Default config (all winners): PSL=4, recur=[2,3,4,5] target=both,
                              gate=on, src=proj, width=8.
Override GATE_WIDTH=12 to test the leaderboard width on top of PR×DR.

The sweep axis is just SEEDS (default 1337). Other knobs are scalar env vars,
not sweeps — this script validates a composition, it does not re-explore.
"""
...
# Friend's GPTQ + LQER quantization (replaces INT8 when USE_GPTQ_LQER=1).
# This module's CONTROL_TENSOR_NAME_PATTERNS doesn't include our PR/Gate names
# (attn_gate_proj, parallel_post, parallel_resid), but all of those tensors are
# under its 65 536-numel small-tensor passthrough threshold and so are kept as
# fp16 verbatim — no modification of gptq_lqer required.
from gptq_lqer import gptq_lqer_quantize_state_dict, dequantize_gptq_lqer
...
class Hyperparameters:
    ...
    # ---- GPTQ + LQER compression (friend's work, integrated) ----
    # USE_GPTQ_LQER=1 → asymmetric group-wise int6 for attn/MLP + int7 for embeddings
    #                    + LQER rank-4 correction on the top-k most sensitive layers.
    # USE_GPTQ_LQER=0 → fall back to the baseline INT8 quantization (matches the original
    #                    abl8 protocol; useful for like-for-like comparisons).
    # All other knobs apply only when USE_GPTQ_LQER=1.
    use_gptq_lqer = bool(int(os.environ.get("USE_GPTQ_LQER", "1")))
    matrix_quant_bits = int(os.environ.get("MATRIX_QUANT_BITS", "6"))
    embed_quant_bits = int(os.environ.get("EMBED_QUANT_BITS", "7"))
    ...
    quant_asymmetric = bool(int(os.environ.get("QUANT_ASYMMETRIC", "1")))
    # Calibration: how many seqs from val_tokens to use for Hessian collection.
    quant_calibration_seqs = int(os.environ.get("QUANT_CALIBRATION_SEQS", "128"))
...
def run_one(
    ...
    if master_process:
        ...
        run_config.update({
            ...
            # Quantization config
            "use_gptq_lqer": args.use_gptq_lqer,
            "matrix_quant_bits": args.matrix_quant_bits if args.use_gptq_lqer else 8,
            "embed_quant_bits": args.embed_quant_bits if args.use_gptq_lqer else 8,
            "lqer_rank": args.lqer_rank if args.use_gptq_lqer else 0,
            "lqer_top_k": args.lqer_top_k if args.use_gptq_lqer else 0,
            "quant_group_size": args.quant_group_size if args.use_gptq_lqer else 0,
            "quant_asymmetric": args.quant_asymmetric if args.use_gptq_lqer else False,
        })
        quant_tag = (
            f"gptqlqer_m{args.matrix_quant_bits}e{args.embed_quant_bits}"
            f"_r{args.lqer_rank}_top{args.lqer_top_k}_g{args.quant_group_size}"
            if args.use_gptq_lqer else "int8"
        )
        wandb.init(
            ...
            name=run_id,
            config=run_config,
            tags=["combined_pr_dr_gate_gptqlqer", psl_tag, f"target_{args.recur_target}",
    ...
    # Serialisation + quantised final eval — switchable between GPTQ+LQER and INT8.
    quant_suffix = "gptqlqer" if args.use_gptq_lqer else "int8"
    model_quant_path = f"final_model_{run_id}.{quant_suffix}.ptz"

    if args.use_gptq_lqer:
        log0(f"[QUANT] Running GPTQ+LQER  (matrix_bits={args.matrix_quant_bits} "
             f"embed_bits={args.embed_quant_bits} lqer_rank={args.lqer_rank} "
             f"lqer_top_k={args.lqer_top_k} group_size={args.quant_group_size} "
             f"asymmetric={args.quant_asymmetric})")
        quant_obj, quant_stats = gptq_lqer_quantize_state_dict(
            model=base_model,
            calibration_tokens=val_tokens,
            matrix_bits=args.matrix_quant_bits,
            embed_bits=args.embed_quant_bits,
            lqer_rank=args.lqer_rank,
            lqer_top_k=args.lqer_top_k,
            group_size=args.quant_group_size,
            asymmetric=args.quant_asymmetric,
            num_calibration_seqs=args.quant_calibration_seqs,
            seq_len=args.train_seq_len,
            device=device,
        )
        dequant_fn = dequantize_gptq_lqer
    else:
        log0("[QUANT] Running baseline INT8 quantization")
    ...
    if master_process:
        ...
        log0(f"Serialised {quant_suffix}+zlib: {quant_file_bytes} bytes  "
...
def main() -> None:
    ...
    if master_process:
        ...
        print(f"  use_gptq_lqer         : {args.use_gptq_lqer}")
        if args.use_gptq_lqer:
            print(f"  matrix_quant_bits     : {args.matrix_quant_bits}")
            print(f"  embed_quant_bits      : {args.embed_quant_bits}")
            print(f"  lqer_rank             : {args.lqer_rank}")
            print(f"  lqer_top_k            : {args.lqer_top_k}")
            print(f"  quant_group_size      : {args.quant_group_size}")
            print(f"  quant_asymmetric      : {args.quant_asymmetric}")
        print(f"  --- training ---")
