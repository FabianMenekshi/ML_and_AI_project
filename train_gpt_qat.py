...
def fake_quantize_asymmetric(w: Tensor, bits: int = 6, group_size: int = 32) -> Tensor:
    ...
    if padded > in_features:
        w_padded = F.pad(w, (0, padded - in_features), value=0.0)
    else:
        w_padded = w
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        while True:
            ...
            if QAT_ENABLED and not CastedLinear._qat_enabled:
                qat_start_step = int(QAT_START_FRACTION * args.iterations)
                if step >= qat_start_step:
                    CastedLinear._qat_enabled = True
                    CastedLinear._qat_bits = QAT_BITS
                    CastedLinear._qat_group_size = QAT_GROUP_SIZE
                    log0(f"[QAT] Enabled at step {step}/{args.iterations} "
                         f"(fraction={QAT_START_FRACTION}, bits={QAT_BITS}, group_size={QAT_GROUP_SIZE})")
                    # Recompile to pick up the new QAT branch in CastedLinear
                    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
