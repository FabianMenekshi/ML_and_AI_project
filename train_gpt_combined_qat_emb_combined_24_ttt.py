...
class Hyperparameters:
    ...
    # ---- Score-first LoRA test-time training (TTT) ----
    # This is disabled by default so normal training/validation is unchanged.
    # When enabled, final validation can adapt temporary LoRA adapters after each
    # chunk has already been scored. The base model weights are frozen.
    ttt_enabled = bool(int(os.environ.get("TTT_ENABLED", "0")))
    ttt_rank = int(os.environ.get("TTT_RANK", "4"))
    ttt_alpha = float(os.environ.get("TTT_ALPHA", str(ttt_rank)))
    ttt_lr = float(os.environ.get("TTT_LR", "0.005"))
    ttt_epochs_per_chunk = int(os.environ.get("TTT_EPOCHS_PER_CHUNK", "1"))
    ttt_chunk_len = int(os.environ.get("TTT_CHUNK_LEN", str(train_seq_len)))
    ttt_bos_token_id = int(os.environ.get("TTT_BOS_TOKEN_ID", "1"))
    ttt_target_lm_head = bool(int(os.environ.get("TTT_TARGET_LM_HEAD", "1")))
    ttt_target_cq = bool(int(os.environ.get("TTT_TARGET_CQ", "1")))
    ttt_target_cv = bool(int(os.environ.get("TTT_TARGET_CV", "1")))
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Optional score-first TTT LoRA branch. These attributes are only attached
        # for final TTT evaluation, not during normal training or serialization.
        # Base path remains unchanged when no adapter is present.
        if getattr(self, "ttt_lora_enabled", False):
            a = self.ttt_lora_A.to(dtype=x.dtype)       # [rank, in_features]
            b = self.ttt_lora_B.to(dtype=x.dtype)       # [out_features, rank]
            lora = F.linear(F.linear(x, a), b)
            out = out + lora * float(self.ttt_lora_scale)
        return out
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            ...
            # Optional LoRA adapter for the tied LM head. This deliberately does
            # not modify tok_emb itself; it adds a temporary low-rank output-head
            # correction used only during score-first TTT evaluation.
            if getattr(self, "ttt_lm_head_lora_enabled", False):
                a = self.ttt_lm_head_lora_A.to(dtype=x.dtype)
                b = self.ttt_lm_head_lora_B.to(dtype=x.dtype)
                logits_proj = logits_proj + F.linear(F.linear(x, a), b) * float(self.ttt_lm_head_lora_scale)
        else:
...
def _make_lora_pair(
    ...
    """Attach a temporary LoRA adapter to parent and return its trainable params."""
    if rank <= 0:
        raise ValueError(f"TTT_RANK must be positive, got {rank}")
    ...
    nn.init.normal_(a, mean=0.0, std=1.0 / math.sqrt(max(in_features, 1)))
    parent.register_parameter(f"{name}_A", a)
    parent.register_parameter(f"{name}_B", b)
    parent.register_buffer(f"{name}_A_init", a.detach().clone(), persistent=False)
    parent.register_buffer(f"{name}_B_init", b.detach().clone(), persistent=False)
    setattr(parent, f"{name}_scale", float(scale) / float(rank))
    setattr(parent, f"{name}_enabled", True)
    return [a, b]


def attach_score_first_lora_ttt(model: GPT, args: Hyperparameters, device: torch.device) -> list[nn.Parameter]:
    """Attach temporary LoRA adapters for score-first TTT.

    Targets:
      - attention c_q and c_v in every block, controlled by TTT_TARGET_CQ/CV
      - tied lm_head via a separate logits correction, controlled by TTT_TARGET_LM_HEAD
        If embeddings are untied, the adapter is attached to model.lm_head instead.
    """
    params: list[nn.Parameter] = []
    rank = args.ttt_rank
    scale = args.ttt_alpha

    for block in model.blocks:
        if args.ttt_target_cq:
            params.extend(_make_lora_pair(
                "ttt_lora", block.attn.c_q, block.attn.c_q.in_features, block.attn.c_q.out_features,
                rank, scale, device,
            ))
        if args.ttt_target_cv:
            params.extend(_make_lora_pair(
                "ttt_lora", block.attn.c_v, block.attn.c_v.in_features, block.attn.c_v.out_features,
                rank, scale, device,
            ))

    if args.ttt_target_lm_head:
        if model.tie_embeddings:
            params.extend(_make_lora_pair(
                "ttt_lm_head_lora", model, model.tok_emb.embedding_dim, model.tok_emb.num_embeddings,
                rank, scale, device,
            ))
        else:
            if model.lm_head is None:
                raise RuntimeError("lm_head is required for untied-head TTT")
            params.extend(_make_lora_pair(
                "ttt_lora", model.lm_head, model.lm_head.in_features, model.lm_head.out_features,
                rank, scale, device,
            ))

    # Freeze base model weights and train only the temporary LoRA parameters.
...
@torch.no_grad()
def reset_score_first_lora_ttt(model: nn.Module) -> None:
    """Reset temporary LoRA adapters at document boundaries."""
    for module in model.modules():
        if getattr(module, "ttt_lora_enabled", False):
            module.ttt_lora_A.copy_(module.ttt_lora_A_init)
            module.ttt_lora_B.copy_(module.ttt_lora_B_init)
        if getattr(module, "ttt_lm_head_lora_enabled", False):
            module.ttt_lm_head_lora_A.copy_(module.ttt_lm_head_lora_A_init)
            module.ttt_lm_head_lora_B.copy_(module.ttt_lm_head_lora_B_init)


@torch.no_grad()
def score_first_lora_ttt_norm(params: list[nn.Parameter]) -> float:
    if not params:
        return 0.0
    return float(sum(p.detach().float().norm().item() ** 2 for p in params) ** 0.5)


def eval_val_score_first_lora_ttt(
    args: Hyperparameters,
    model: GPT,
    rank: int,
    world_size: int,
    ...
    """Validation with document-reset, score-first LoRA TTT.

    The loss for every chunk is added to the metric before any gradient step is
    taken on that chunk. LoRA adapters are reset whenever a BOS token starts a
    new chunk, giving document-local adaptation without changing base weights.
    """
    if not args.ttt_enabled:
        raise RuntimeError("eval_val_score_first_lora_ttt called while TTT_ENABLED=0")
    if world_size != 1 and dist.is_available() and dist.is_initialized():
        # Distributed is supported by reducing sums below, but each rank owns a
        # disjoint validation span and therefore has its own local TTT state.
        pass

    was_training = model.training
    old_qat_enabled = CastedLinear._qat_enabled
    CastedLinear._qat_enabled = False
    model.eval()

    params = attach_score_first_lora_ttt(model, args, device)
    optimizer = torch.optim.SGD(params, lr=args.ttt_lr)
    reset_score_first_lora_ttt(model)

    chunk_len = max(1, min(int(args.ttt_chunk_len), int(args.train_seq_len)))
    total_seqs = (val_tokens.numel() - 1) // args.train_seq_len
    seq_start = (total_seqs * rank) // world_size
    seq_end = (total_seqs * (rank + 1)) // world_size
    raw_start = seq_start * args.train_seq_len
    raw_end = seq_end * args.train_seq_len + 1
    local_tokens = val_tokens[raw_start:raw_end].to(device=device, dtype=torch.int64, non_blocking=True)

    val_loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    val_token_count = torch.zeros((), device=device, dtype=torch.float64)
    val_byte_count = torch.zeros((), device=device, dtype=torch.float64)
    loss_for_ttt_sum = torch.zeros((), device=device, dtype=torch.float64)
    chunks = 0
    resets = 0

    pos = 0
    while pos < local_tokens.numel() - 1:
        if int(local_tokens[pos].item()) == int(args.ttt_bos_token_id) or pos == 0:
            reset_score_first_lora_ttt(model)
            optimizer = torch.optim.SGD(params, lr=args.ttt_lr)
            resets += 1

        end = min(pos + chunk_len, local_tokens.numel() - 1)
        # Avoid a chunk input that straddles a BOS boundary. The BOS itself may
        # still be scored as a target at the end of the previous document, matching
        # the token stream metric, then the adapter resets before BOS becomes input.
        if end > pos + 1:
            rel_bos = (local_tokens[pos + 1:end] == int(args.ttt_bos_token_id)).nonzero(as_tuple=False)
            if rel_bos.numel() > 0:
                end = pos + 1 + int(rel_bos[0].item())
        if end <= pos:
            end = pos + 1

        x = local_tokens[pos:end].unsqueeze(0)
        y = local_tokens[pos + 1:end + 1].unsqueeze(0)

        # 1) SCORE FIRST: metric is computed before any update on this chunk.
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            scored_loss = model(x, y).detach()
        token_count = float(y.numel())
        val_loss_sum += scored_loss.to(torch.float64) * token_count
        ...
        # 2) THEN TRAIN: adapt temporary LoRA params only, for future chunks.
        for _ in range(max(0, int(args.ttt_epochs_per_chunk))):
            optimizer.zero_grad(set_to_none=True)
            with torch.enable_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ttt_loss = model(x, y)
            ttt_loss.backward()
            optimizer.step()
            loss_for_ttt_sum += ttt_loss.detach().to(torch.float64)

        chunks += 1
        if log0 is not None and args.ttt_log_every_chunks > 0 and chunks % args.ttt_log_every_chunks == 0:
            log0(f"[TTT] rank:{rank} chunks:{chunks} tokens:{int(val_token_count.item())} "
                 f"score_loss:{float(scored_loss.item()):.4f} lora_norm:{score_first_lora_ttt_norm(params):.4f}")
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        dist.all_reduce(val_token_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(val_byte_count, op=dist.ReduceOp.SUM)
        loss_for_ttt_sum_tensor = loss_for_ttt_sum.clone()
        chunks_tensor = torch.tensor(float(chunks), device=device, dtype=torch.float64)
        resets_tensor = torch.tensor(float(resets), device=device, dtype=torch.float64)
        dist.all_reduce(loss_for_ttt_sum_tensor, op=dist.ReduceOp.SUM)
        dist.all_reduce(chunks_tensor, op=dist.ReduceOp.SUM)
        dist.all_reduce(resets_tensor, op=dist.ReduceOp.SUM)
        chunks_total = float(chunks_tensor.item())
        resets_total = float(resets_tensor.item())
        train_loss_total = float(loss_for_ttt_sum_tensor.item())
    else:
        chunks_total = float(chunks)
        resets_total = float(resets)
        train_loss_total = float(loss_for_ttt_sum.item())

    val_loss = val_loss_sum / val_token_count
    bits_per_token = val_loss.item() / math.log(2.0)
    tokens_per_byte = val_token_count.item() / val_byte_count.item()
    stats = {
        "ttt_chunks": chunks_total,
        "ttt_resets": resets_total,
        "ttt_lora_param_count": float(sum(p.numel() for p in params)),
        "ttt_lora_norm_final": score_first_lora_ttt_norm(params),
        "ttt_train_loss_mean": train_loss_total / max(chunks_total * max(1, args.ttt_epochs_per_chunk), 1.0),
    }

    # Keep adapters attached but disabled from future accidental use.
    for module in model.modules():
        if getattr(module, "ttt_lora_enabled", False):
            module.ttt_lora_enabled = False
        if getattr(module, "ttt_lm_head_lora_enabled", False):
            module.ttt_lm_head_lora_enabled = False
    for p in model.parameters():
        p.requires_grad_(True)
    CastedLinear._qat_enabled = old_qat_enabled
...
def run_one(
    ...
    ttt_val_loss = None
    ttt_val_bpb = None
    ttt_stats: dict[str, float] = {}
    if args.ttt_enabled:
        if distributed:
            dist.barrier()
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        # Use the uncompiled base_model for TTT: adapters are attached after
        # training/quantization and should not become part of torch.compile's
        # fixed graph. The quantized round-trip weights are already loaded above.
        ttt_val_loss, ttt_val_bpb, ttt_stats = eval_val_score_first_lora_ttt(
            args, base_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            log0=log0,
        )
        torch.cuda.synchronize()
        log0(f"final_score_first_lora_ttt val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} "
             f"eval_time:{1000.0*(time.perf_counter()-t_ttt):.0f}ms "
             f"chunks:{ttt_stats.get('ttt_chunks', 0):.0f} resets:{ttt_stats.get('ttt_resets', 0):.0f}")
    ...
    if master_process:
        ...
        if ttt_val_loss is not None and ttt_val_bpb is not None:
            final_log.update({
                "final_ttt_val_loss": ttt_val_loss,
                "final_ttt_val_bpb": ttt_val_bpb,
                "final_ttt_bpb_delta_vs_roundtrip": ttt_val_bpb - q_val_bpb,
                **ttt_stats,
            })
        wandb.log(final_log)
        wandb.finish()
