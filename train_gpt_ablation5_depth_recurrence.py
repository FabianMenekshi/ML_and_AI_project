...
Identical to train_gpt.py except:
  - GPT.forward repeats selected blocks (shared weights, no extra params).
  - Sweep axis: RECUR_LAYERS_VALUES (semicolon-separated layer sets, e.g. "4;4,5;3,4").
  - RECUR_TIMES extra passes per recurred block (default 1 → each block runs twice).
"""
...
class Hyperparameters:
    ...
    # Ablation-specific: semicolon-separated layer sets, e.g. "4;4,5;3,4"
    recur_layers_values: list[list[int]] = [
        [int(x) for x in s.split(",")]
        for s in os.environ.get("RECUR_LAYERS_VALUES", "4,5").split(";")
    ]
    # Extra passes per recurred block (1 → block runs twice total)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Store as frozenset so torch.compile can treat them as compile-time constants.
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            # Repeat the block with shared weights (no extra params).
            # Skip connections are NOT re-added on repeated passes.
...
def run_one(
    ...
    layers_tag = "_".join(str(l) for l in sorted(recur_layers))
    run_id = f"{args.base_run_id}_layers{layers_tag}_seed{seed}"
    ...
    log0(f"\n{'=' * 100}")
    log0(f"Starting run: recur_layers={sorted(recur_layers)} recur_times={args.recur_times} seed={seed}  run_id={run_id}")
    log0(f"{'=' * 100}")
    ...
    log0(f"model_params:{n_params}  recur_layers:{sorted(recur_layers)}  recur_times:{args.recur_times}")

    if master_process:
        run_config = {k: getattr(args, k) for k in vars(Hyperparameters) if not k.startswith("_")}
        run_config["seed"] = seed
        run_config["run_id"] = run_id
        run_config["recur_layers"] = sorted(recur_layers)
    ...
    # Quantization + roundtrip validation
...
def main() -> None:
    ...
    for recur_layers in args.recur_layers_values:
        for seed in args.seeds:
            run_one(args=args, recur_layers=recur_layers, seed=seed, **shared)
