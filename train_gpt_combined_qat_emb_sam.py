"""
COMBINED + SAM-IN-QAT: PR × DR × Attention Gate + Mixed int6/int8 + QAT
                       + Sharpness-Aware Minimization (SAM) during the QAT phase.

Identical to train_gpt_combined_qat_emb.py with one training-step modification:
  - For iters 0 .. qat_start_step the training step is unchanged (one forward+backward
    per micro-step, accumulated, then optimizer.step()).
  - For iters >= qat_start_step the training step becomes a SAM step:
      1. Forward+backward on the original weights (accumulated over micro-steps) to get g.
      2. Perturb θ ← θ + ρ · g / ||g||  (an inner gradient *ascent* step).
      3. Forward+backward at the perturbed weights to get g'.
      4. Restore θ ← θ - ρ · g / ||g||  (revert the perturbation).
      5. optimizer.step() using g' (gradient computed at the "uphill" point).
    This biases optimisation toward flatter minima — the model is rewarded for
    having low loss in a neighbourhood, not just at a point. Empirically this
    correlates with quantisation robustness because flat minima are robust to
    the rounding noise that quantisation injects.

Cost: SAM doubles the forward+backward count during the QAT phase, so the
run takes ~75% longer than baseline (QAT is 75% of training; SAM adds 1× to that).

We replay the same micro-batches in Phase 3 so the gradient at the perturbed
point is comparable to the gradient at the original point. Micro-batches are
cached in CPU memory between Phase 1 and Phase 3 (cost: a few MB).

Env vars:
  SAM_RHO          (default 0.05)  — perturbation radius (∥ρ · g / ∥g∥∥ in weight space)
  SAM_ADAPTIVE     (default 0)     — if 1, use Adaptive SAM (ASAM) with per-param scaling
"""
...
class Hyperparameters:
    ...
    # ---- Sharpness-Aware Minimization (SAM) — active only during QAT phase ----
    sam_rho = float(os.environ.get("SAM_RHO", "0.05"))
    sam_adaptive = bool(int(os.environ.get("SAM_ADAPTIVE", "0")))
...
def run_one(
    ...
    # from step 0, AND making SAM active from step 0 instead of from qat_start_step
    # (since sam_active is tied to _qat_enabled).
    ...
    while True:
        ...
        if args.qat_enabled and not CastedLinear._qat_enabled:
            ...
            if step >= qat_start_step:
                ...
                model = DDP(compiled_model, device_ids=[local_rank],
                            broadcast_buffers=False) if distributed else compiled_model
                log0(f"[SAM] Activated alongside QAT  rho={args.sam_rho}  adaptive={args.sam_adaptive}  "
                     f"(each remaining step now does 2 forward+backward passes)")
                if master_process:
                    wandb.log({"qat_activated": 1, "sam_activated": 1,
                               "tokens_seen": step * args.train_batch_tokens}, step=step)

        zero_grad_all()
        train_loss = torch.zeros((), device=device)

        # SAM is active iff QAT is active. During QAT phase we cache micro-batches
        # so we can replay them at the perturbed point.
        sam_active = CastedLinear._qat_enabled
        cached_batches: list[tuple[Tensor, Tensor]] = []
        sam_grad_norm = 0.0  # populated below when sam_active
        ...
        # ---- SAM inner step (only during QAT phase) ----
        if sam_active:
            # Compute the global gradient norm over trainable matrix + scalar params.
            # We exclude embeddings and lm_head from SAM perturbation: their gradients
            # have very different scales and would dominate the ρ-normalised direction.
            sam_param_groups = [matrix_params, scalar_params]
            sam_params = [p for group in sam_param_groups for p in group]
            with torch.no_grad():
                sam_grad_sqnorm = sum(
                    (p.grad.float().norm() ** 2).item()
                    for p in sam_params if p.grad is not None
                )
                sam_grad_norm = sam_grad_sqnorm ** 0.5
                sam_scale = args.sam_rho / (sam_grad_norm + 1e-12)

                # Perturb θ ← θ + scale · g (ascent step). Stash perturbation so we can revert.
                perturbations: dict[int, Tensor] = {}
                for p in sam_params:
                    if p.grad is None:
                        continue
                    if args.sam_adaptive:
                        # ASAM: scale by |θ| element-wise (parameter-aware perturbation).
                        e = sam_scale * p.detach().abs() * p.grad
                    else:
                        e = sam_scale * p.grad
                    e = e.to(dtype=p.dtype)
                    perturbations[id(p)] = e
                    p.data.add_(e)

            # Zero grads, replay micro-batches at the perturbed point.
            zero_grad_all()
            for micro_step, (x, y) in enumerate(cached_batches):
                if distributed:
                    model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    loss_perturbed = model(x, y)
                (loss_perturbed * grad_scale).backward()

            # Restore θ ← θ - scale · g.
            with torch.no_grad():
                for p in sam_params:
                    e = perturbations.get(id(p))
                    if e is not None:
                        p.data.sub_(e)
        ...
        if should_log:
            ...
            if master_process:
                log_dict = {
                    ...
                    "sam_active": int(sam_active),
                    "sam_grad_norm": sam_grad_norm if sam_active else 0.0,
