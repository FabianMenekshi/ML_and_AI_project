"""
COMBINED + SCORE-FIRST TTT-EVAL + PER-DOCUMENT SOFT-RESET (novel ablation)

Extends train_gpt_combined_qat_emb_tttev.py with a new mechanism that has NOT been
seen on the parameter-golf leaderboard: at SentencePiece document boundaries, the
TTT-adapted weights are blended back toward the original (pre-TTT) weights with a
tunable mixing coefficient α:

    θ ← α · θ_original  +  (1 - α) · θ_current      (only at boundary tokens)

Three regimes of α:
    α = 0.0   →  no reset           (reproduces the standard tttev script)
    α = 1.0   →  hard reset         (each document starts adaptation from scratch)
    α ∈ (0,1) →  soft reset         (partially forget prior-document adaptation)

Motivation: FineWeb concatenates documents from very different distributions
(Wikipedia, code, news, fiction, recipes). Continuous-drift TTT lets adaptation
to document A actively hurt scoring of document B. Soft-reset at boundaries
gives each document its own clean adaptation budget, parameterised by α.

This script runs ONE training pass + N TTT-eval passes back-to-back (one per α in
the TTT_EVAL_RESET_ALPHAS list). Each TTT-eval pass starts from the same fresh
dequantised state (snapshotted once after the standard final eval). Training cost
is amortised across all α values.

Each per-α result is logged separately as `final_val_bpb_tttev_alpha_{X}` for
later sweep analysis.

Env vars (new):
  TTT_EVAL_RESET_ALPHAS  (default "0.0,0.25,0.5,0.75,1.0")
                         comma-separated α values to sweep in one run

Env vars (inherited from _tttev.py):
  TTT_EVAL_ENABLED, TTT_EVAL_LR, TTT_EVAL_STEPS, TTT_EVAL_OPTIMIZER, TTT_EVAL_RESET
"""
...
class Hyperparameters:
    ...
    # ---- Score-first TTT-eval (post-quant, optional second evaluation pass) ----
    # Operates on the DEQUANTISED weights after the standard final_val_bpb is recorded.
    # The base 16 MB artifact is unchanged; gradients only modify the in-memory model.
    ttt_eval_enabled = bool(int(os.environ.get("TTT_EVAL_ENABLED", "1")))
    ttt_eval_lr = float(os.environ.get("TTT_EVAL_LR", "1e-4"))
    ttt_eval_steps = int(os.environ.get("TTT_EVAL_STEPS", "1"))
    ttt_eval_optimizer = os.environ.get("TTT_EVAL_OPTIMIZER", "adamw").lower()
    ttt_eval_reset = bool(int(os.environ.get("TTT_EVAL_RESET", "1")))
    # ---- NEW: per-document soft-reset sweep ----
    # Comma-separated list of α values to sweep in a single run.
    # α = 0.0 → no reset (identical to the standard tttev script).
    # α = 1.0 → hard reset to original weights at every document boundary.
    # 0 < α < 1 → soft blend: θ ← α·θ_original + (1-α)·θ_current at boundaries.
    ttt_eval_reset_alphas = [
        float(s) for s in os.environ.get("TTT_EVAL_RESET_ALPHAS", "0.0,0.25,0.5,0.75,1.0").split(",")
        if s.strip()
    ]
...
def _build_eval_optimizer(model: nn.Module, optimizer_name: str, lr: float):
    if optimizer_name == "adamw":
        return torch.optim.AdamW(model.parameters(), lr=lr, betas=(0.9, 0.95),
                                 weight_decay=0.0, fused=True)
    if optimizer_name == "sgd":
        return torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9)
    raise ValueError(f"Unknown TTT_EVAL_OPTIMIZER {optimizer_name!r} (expected 'adamw' or 'sgd').")


def eval_val_with_score_first_ttt(
    args: "Hyperparameters",
    model: nn.Module,
    rank: int,
    world_size: int,
    ...
    n_steps: int,
    optimizer_name: str,
    reset_alpha: float = 0.0,
    original_state: dict[str, Tensor] | None = None,
    log_fn=print,
) -> tuple[float, float, int]:
    """Score-first TTT-eval with optional per-document soft-reset.

    For each validation batch:
        Phase 1 (SCORE):     no_grad forward pass → loss counted toward BPB.
        Phase 2 (UPDATE):    forward + backward + optimizer.step(), n_steps times.
        Phase 3 (BOUNDARY):  if any token in this batch is a SentencePiece document
                             boundary AND reset_alpha > 0:
                                 θ ← α·θ_original + (1-α)·θ_current   (in-place)
                                 rebuild the optimizer (fresh momentum)
    Causality is preserved because the score for batch i is computed with weights
    only trained on batches 0..i-1.

    Single-process only — distributed TTT-eval would need per-rank gradient sync,
    which we don't bother with since the sbatch always uses --gpus=1.

    Returns: (val_loss, val_bpb, n_resets)
    """
    if dist.is_available() and dist.is_initialized() and world_size > 1:
        raise NotImplementedError(
            "score-first TTT-eval only supports single-GPU; gradient sync across "
            "ranks is not implemented (and would invalidate causality anyway)."
        )
    if reset_alpha > 0 and original_state is None:
        raise ValueError("reset_alpha > 0 requires original_state to be provided.")

    opt = _build_eval_optimizer(model, optimizer_name, lr)

    # Disable QAT fake-quant during TTT-eval: the weights are already dequantised
    # int6/int8 stored as bf16/fp32; re-quantising them per forward pass would
    # both fight the optimizer and be a no-op on the rounded values.
    ...
    model.train()  # we need grads for Phase 2
    n_batches = 0
    n_resets = 0
    score_losses_log: list[float] = []
    ...
    for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
        ...
        token_bytes += (has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]).to(dtype=torch.int16)
        val_byte_count += token_bytes.to(torch.float64).sum()
        score_losses_log.append(float(batch_loss.item()))

        # ---- Phase 2: UPDATE (with grad) ----
        for _ in range(n_steps):
            opt.zero_grad(set_to_none=True)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                update_loss = model(x, y)
            update_loss.backward()
            opt.step()
        n_batches += 1

        # ---- Phase 3: per-document SOFT RESET (only when reset_alpha > 0) ----
        # If any TARGET token in this batch was a SentencePiece document boundary,
        # blend the live weights back toward the original with mixing α and
        # rebuild the optimizer (fresh momentum). α=1 is a hard reset; α=0 is no
        # reset (the existing tttev behaviour).
        if reset_alpha > 0:
            with torch.no_grad():
                has_boundary = bool(is_boundary_token_lut[y.reshape(-1)].any().item())
            if has_boundary:
                with torch.no_grad():
                    for name, p in model.named_parameters():
                        if name in original_state:
                            p.data.mul_(1.0 - reset_alpha).add_(original_state[name],
                                                                alpha=reset_alpha)
                opt = _build_eval_optimizer(model, optimizer_name, lr)
                n_resets += 1

        # Occasional progress log so the slurm output shows the curve
        if n_batches <= 5 or n_batches % 20 == 0:
            ema_score = sum(score_losses_log[-5:]) / max(len(score_losses_log[-5:]), 1)
            log_fn(f"[TTT-EVAL] batch {n_batches:4d}  score_loss={float(batch_loss.item()):.4f}  "
                   f"recent_5_mean={ema_score:.4f}  n_resets={n_resets}")

    # Restore QAT flag (no-op if it was off).
    CastedLinear._qat_enabled = prev_qat

    val_loss = val_loss_sum / val_token_count
    bits_per_token = val_loss.item() / math.log(2.0)
    tokens_per_byte = val_token_count.item() / val_byte_count.item()
    return float(val_loss.item()), float(bits_per_token * tokens_per_byte), n_resets
...
def run_one(
    ...
    # ---- Score-first TTT-eval with per-document soft-reset SWEEP ----
    # Runs N TTT-eval passes (one per α in args.ttt_eval_reset_alphas), each
    # starting from the same fresh dequantised state. Training cost is amortised
    # across all α values. Each per-α result is logged separately.
    if args.ttt_eval_enabled and args.ttt_eval_reset_alphas:
        # Snapshot the dequantised state ONCE; we restore from it before each pass.
        ttt_eval_backup = {k: v.detach().clone() for k, v in base_model.state_dict().items()}
        # original_state for the soft-reset (must be on the model's device, fp32-safe).
        original_state = {k: v.detach().clone() for k, v in base_model.state_dict().items()}

        tttev_results: dict[float, dict] = {}
        for alpha in args.ttt_eval_reset_alphas:
            # Restore weights to the pristine dequantised state before this α-pass.
            base_model.load_state_dict(ttt_eval_backup, strict=True)

            log0(f"[TTT-EVAL] α={alpha:.2f}  starting score-first TTT-eval  "
                 f"lr={args.ttt_eval_lr}  steps={args.ttt_eval_steps}  "
                 f"optimizer={args.ttt_eval_optimizer}")
            torch.cuda.synchronize()
            t_ttt = time.perf_counter()
            tttev_loss, tttev_bpb, n_resets = eval_val_with_score_first_ttt(
                args, base_model, rank, world_size, device, grad_accum_steps,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                lr=args.ttt_eval_lr,
                n_steps=args.ttt_eval_steps,
                optimizer_name=args.ttt_eval_optimizer,
                reset_alpha=alpha,
                original_state=original_state if alpha > 0 else None,
                log_fn=log0,
            )
            torch.cuda.synchronize()
            elapsed_s = time.perf_counter() - t_ttt
            log0(f"[TTT-EVAL] α={alpha:.2f}  final_val_bpb_tttev:{tttev_bpb:.4f}  "
                 f"val_loss:{tttev_loss:.4f}  Δ_vs_base:{tttev_bpb - q_val_bpb:+.4f}  "
                 f"n_resets={n_resets}  eval_time:{elapsed_s:.0f}s")
            tttev_results[alpha] = {
                "bpb": tttev_bpb,
                "loss": tttev_loss,
                "delta_vs_base": tttev_bpb - q_val_bpb,
                "n_resets": n_resets,
                "eval_time_s": elapsed_s,
            }

        # Log all α results to wandb with α-tagged metric names.
        if master_process:
            log_dict = {
                "tttev_lr": args.ttt_eval_lr,
                "tttev_steps_per_batch": args.ttt_eval_steps,
                "tttev_optimizer": args.ttt_eval_optimizer,
                "tttev_alphas_swept": ",".join(f"{a:.2f}" for a in args.ttt_eval_reset_alphas),
            }
            for alpha, res in tttev_results.items():
                tag = f"alpha_{alpha:.2f}".replace(".", "_")
                log_dict[f"final_val_bpb_tttev_{tag}"] = res["bpb"]
                log_dict[f"final_val_loss_tttev_{tag}"] = res["loss"]
                log_dict[f"tttev_{tag}_delta_vs_base"] = res["delta_vs_base"]
                log_dict[f"tttev_{tag}_n_resets"] = res["n_resets"]
                log_dict[f"tttev_{tag}_eval_time_s"] = res["eval_time_s"]
            # Convenience: best α and its BPB
            best_alpha = min(tttev_results, key=lambda a: tttev_results[a]["bpb"])
            log_dict["tttev_best_alpha"] = best_alpha
            log_dict["tttev_best_bpb"] = tttev_results[best_alpha]["bpb"]
            log_dict["tttev_best_delta_vs_base"] = tttev_results[best_alpha]["delta_vs_base"]
            wandb.log(log_dict)

        # Final restore so any post-eval code sees pristine weights.
        base_model.load_state_dict(ttt_eval_backup, strict=True)
        del ttt_eval_backup, original_state
