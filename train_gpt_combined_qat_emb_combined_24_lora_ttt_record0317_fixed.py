...
class Hyperparameters:
    ...
    # ---- Score-first LoRA TTT, matching the 2026-03-17_LoRA_TTT defaults ----
    # The training loop is unchanged; this is used only for the final post-quantization eval.
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Optional eval-time LoRA branch used only by score-first TTT.
        if getattr(self, "ttt_lora_enabled", False):
            a = self.ttt_lora_A.to(dtype=x.dtype)
            b = self.ttt_lora_B.to(dtype=x.dtype)
            out = out + F.linear(F.linear(x, a), b) * float(self.ttt_lora_scale)
        return out
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, return_per_token: bool = False) -> Tensor:
        ...
        if return_per_token:
            return per_token_loss.reshape_as(target_ids)
        return per_token_loss.mean()


# -----------------------------
# SCORE-FIRST LORA TTT EVALUATION
# -----------------------------

def _find_docs_from_bos(all_tokens: Tensor, bos_token_id: int, include_next_bos: bool = True) -> list[tuple[int, int]]:
    """Return (start, length) spans for validation documents using BOS boundaries."""
    bos_positions = (all_tokens == bos_token_id).nonzero(as_tuple=False).flatten().tolist()
    if not bos_positions or bos_positions[0] != 0:
        bos_positions = [0] + bos_positions
    docs: list[tuple[int, int]] = []
    n = int(all_tokens.numel())
    for i, start in enumerate(bos_positions):
        end = int(bos_positions[i + 1]) if i + 1 < len(bos_positions) else n
...
def _compute_ttt_chunk_window(ci: int, pred_len: int, num_chunks: int, chunk_size: int, eval_seq_len: int) -> tuple[int, int, int, int]:
    """Return (window_start, window_len, chunk_offset, chunk_len) for one document chunk."""
...
def _zero_optimizer_state(opt: torch.optim.Optimizer) -> None:
...
def _iter_ttt_lora_linear_modules(model: nn.Module, args: Hyperparameters):
    """Yield CastedLinear modules that receive LoRA adapters: c_q, c_v, and optional untied lm_head."""
    for block in getattr(model, "blocks", []):
        if args.ttt_target_cq:
            yield block.attn.c_q
        if args.ttt_target_cv:
            yield block.attn.c_v
    if args.ttt_target_lm_head and not getattr(model, "tie_embeddings", True) and getattr(model, "lm_head", None) is not None:
        yield model.lm_head


def attach_lora_ttt_adapters(model: nn.Module, args: Hyperparameters, device: torch.device) -> list[nn.Parameter]:
    """Attach rank-r LoRA adapters for score-first TTT and freeze the base model."""
    for p in model.parameters():
        p.requires_grad_(False)

    rank = int(args.ttt_lora_rank)
    scale = 1.0 / max(rank, 1)
    lora_params: list[nn.Parameter] = []

    for module in _iter_ttt_lora_linear_modules(model, args):
        in_dim = int(module.in_features)
        out_dim = int(module.out_features)
        module.ttt_lora_A = nn.Parameter(torch.zeros(rank, in_dim, device=device, dtype=torch.float32))
        module.ttt_lora_B = nn.Parameter(torch.zeros(out_dim, rank, device=device, dtype=torch.float32))
        nn.init.normal_(module.ttt_lora_A, mean=0.0, std=1.0 / math.sqrt(max(in_dim, 1)))
        nn.init.zeros_(module.ttt_lora_B)
        module.ttt_lora_scale = scale
        module.ttt_lora_enabled = True
        lora_params.extend([module.ttt_lora_A, module.ttt_lora_B])

    if args.ttt_target_lm_head and getattr(model, "tie_embeddings", True):
        dim = int(model.tok_emb.embedding_dim)
        vocab = int(model.tok_emb.num_embeddings)
        model.ttt_lm_head_lora_A = nn.Parameter(torch.zeros(rank, dim, device=device, dtype=torch.float32))
        model.ttt_lm_head_lora_B = nn.Parameter(torch.zeros(vocab, rank, device=device, dtype=torch.float32))
        nn.init.normal_(model.ttt_lm_head_lora_A, mean=0.0, std=1.0 / math.sqrt(max(dim, 1)))
        nn.init.zeros_(model.ttt_lm_head_lora_B)
        model.ttt_lm_head_lora_scale = scale
        model.ttt_lm_head_lora_enabled = True
        lora_params.extend([model.ttt_lm_head_lora_A, model.ttt_lm_head_lora_B])
...
@torch.no_grad()
def reset_lora_ttt_adapters(model: nn.Module) -> None:
    """Reset one document's LoRA state: random A, zero B, like a fresh adapter."""
    for module in model.modules():
        if isinstance(module, CastedLinear) and getattr(module, "ttt_lora_enabled", False):
            nn.init.normal_(module.ttt_lora_A, mean=0.0, std=1.0 / math.sqrt(max(module.in_features, 1)))
            module.ttt_lora_B.zero_()
    if getattr(model, "ttt_lm_head_lora_enabled", False):
        dim = int(model.tok_emb.embedding_dim)
        nn.init.normal_(model.ttt_lm_head_lora_A, mean=0.0, std=1.0 / math.sqrt(max(dim, 1)))
        model.ttt_lm_head_lora_B.zero_()


@torch.no_grad()
def lora_ttt_norm(model: nn.Module) -> float:
    total = 0.0
    for module in model.modules():
        if isinstance(module, CastedLinear) and getattr(module, "ttt_lora_enabled", False):
            total += float(module.ttt_lora_A.float().pow(2).sum().item())
            total += float(module.ttt_lora_B.float().pow(2).sum().item())
    if getattr(model, "ttt_lm_head_lora_enabled", False):
        total += float(model.ttt_lm_head_lora_A.float().pow(2).sum().item())
        total += float(model.ttt_lm_head_lora_B.float().pow(2).sum().item())
    return total ** 0.5


def eval_val_lora_ttt_record0317(
    args: Hyperparameters,
    model: nn.Module,
    rank: int,
    world_size: int,
    ...
    """Final eval-time LoRA TTT using the 2026-03-17 defaults: rank-8, Adam lr=0.01, chunk=256.

    This implementation is deliberately outside the training loop. It resets LoRA per document,
    scores each chunk before the Adam update, and uses a sliding evaluation window capped at
    TTT_EVAL_SEQ_LEN, default 1024.
    """
    files = sorted(glob.glob(args.val_files))
    all_tokens = torch.cat([load_data_shard(Path(f)) for f in files]).contiguous()
    docs = _find_docs_from_bos(all_tokens, args.ttt_bos_token_id, include_next_bos=True)
    rank_docs = docs[(len(docs) * rank) // world_size : (len(docs) * (rank + 1)) // world_size]

    model.eval()
    CastedLinear._qat_enabled = False
    lora_params = attach_lora_ttt_adapters(model, args, device)
    opt = torch.optim.Adam(lora_params, lr=args.ttt_lora_lr, betas=(args.beta1, args.beta2), eps=1e-10)

    chunk_size = int(args.ttt_chunk_size)
    eval_seq_len = int(args.ttt_eval_seq_len)
    ...
    for doc_start, doc_len in rank_docs:
        reset_lora_ttt_adapters(model)
        _zero_optimizer_state(opt)
        pred_len = doc_len - 1
        num_chunks = (pred_len + chunk_size - 1) // chunk_size
        for ci in range(num_chunks):
            win_start, win_len, chunk_offset, chunk_len = _compute_ttt_chunk_window(
                ci, pred_len, num_chunks, chunk_size, eval_seq_len
            )
            local = all_tokens[doc_start + win_start : doc_start + win_start + win_len + 1].to(
                device=device, dtype=torch.int64, non_blocking=True
            )
            x = local[:-1].unsqueeze(0)
            y = local[1:].unsqueeze(0)

            # Score first. The per-token losses are accumulated before any update on this chunk.
            with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                per_tok = model(x, y, return_per_token=True)[0]
            scored = per_tok[chunk_offset : chunk_offset + chunk_len]
            prev_ids = x[0, chunk_offset : chunk_offset + chunk_len]
            tgt_ids = y[0, chunk_offset : chunk_offset + chunk_len]
            tok_bytes = base_bytes_lut[tgt_ids].to(dtype=torch.float64)
            tok_bytes += (has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]).to(dtype=torch.float64)
            loss_sum += scored.to(dtype=torch.float64).sum()
            byte_sum += tok_bytes.sum()
            token_count += float(chunk_len)
            chunk_count += 1

            # Then train on the already-scored chunk, except after the final document chunk.
            if ci < num_chunks - 1:
                opt.zero_grad(set_to_none=True)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    per_tok_train = model(x, y, return_per_token=True)[0]
                    train_loss = per_tok_train[chunk_offset : chunk_offset + chunk_len].mean()
                old_norm = lora_ttt_norm(model)
                train_loss.backward()
                opt.step()
                new_norm = lora_ttt_norm(model)
                train_steps += 1
                train_loss_sum += float(train_loss.detach().item())
                update_norm_sum += abs(new_norm - old_norm)
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        stats_vec = torch.tensor(
            [chunk_count, train_steps, train_loss_sum, update_norm_sum, float(len(rank_docs))],
            device=device, dtype=torch.float64,
        )
        dist.all_reduce(stats_vec, op=dist.ReduceOp.SUM)
        chunk_count = int(stats_vec[0].item())
        train_steps = int(stats_vec[1].item())
        train_loss_sum = float(stats_vec[2].item())
        update_norm_sum = float(stats_vec[3].item())
        doc_count = int(stats_vec[4].item())
    else:
        doc_count = len(rank_docs)

    val_loss = float(loss_sum.item() / max(token_count.item(), 1.0))
    val_bpb = float((loss_sum.item() / math.log(2.0)) / max(byte_sum.item(), 1.0))
    stats = {
        "ttt/docs": float(doc_count),
        "ttt/chunks": float(chunk_count),
        "ttt/train_steps": float(train_steps),
        "ttt/lora_param_count": float(sum(p.numel() for p in lora_params)),
        "ttt/lora_norm_final": float(lora_ttt_norm(model)),
        "ttt/train_loss_mean": float(train_loss_sum / max(train_steps, 1)),
        "ttt/update_norm_proxy_mean": float(update_norm_sum / max(train_steps, 1)),
        "ttt/lora_rank": float(args.ttt_lora_rank),
        "ttt/lora_lr": float(args.ttt_lora_lr),
        "ttt/chunk_size": float(args.ttt_chunk_size),
        "ttt/eval_seq_len": float(args.ttt_eval_seq_len),
    }
    model.train()
...
def run_one(
    ...
    if args.ttt_enabled:
        ...
        # TTT alternates no_grad scoring with gradient-based adapter updates and
        # uses variable-length sliding windows. Running that path through the
        # torch.compile(fullgraph=True) wrapper causes excessive Dynamo
        # recompilation, so final TTT eval intentionally uses the underlying
        # eager base_model. The normal training loop and normal validation path
        # remain compiled and unchanged.
        ttt_val_loss, ttt_val_bpb, ttt_stats = eval_val_lora_ttt_record0317(
            args, base_model, rank, world_size, device,
            base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        )
        torch.cuda.synchronize()
        ttt_stats["ttt/eval_time_ms"] = 1000.0 * (time.perf_counter() - t_ttt)
        log0(f"final_lora_ttt val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} "
             f"delta_vs_roundtrip:{ttt_val_bpb - q_val_bpb:+.4f} "
             f"eval_time:{ttt_stats['ttt/eval_time_ms']:.0f}ms")
    ...
    if master_process:
        ...
        final_log = {
            ...
            "ttt/enabled": float(args.ttt_enabled),
        }
        if args.ttt_enabled and ttt_val_loss is not None and ttt_val_bpb is not None:
            final_log.update({
                "final_ttt_val_loss": ttt_val_loss,
                "final_ttt_val_bpb": ttt_val_bpb,
                "final_ttt_bpb_delta_vs_roundtrip": ttt_val_bpb - q_val_bpb,
            })
            final_log.update(ttt_stats)
        wandb.log(final_log)
        wandb.finish()
