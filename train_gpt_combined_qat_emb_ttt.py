"""
COMBINED + TTT: PR × DR × Attention Gate + Mixed int6/int8 + QAT
                + Test-Time Training (TTT-Linear, Sun et al. 2024) at pre-PR encoder layers.

TTT-Linear adds a new sublayer to selected encoder blocks. Inside each TTT layer
a small hidden state W of shape (d_state, d_state) is updated ONLINE — via
gradient descent on a self-supervised reconstruction objective — as the
...
Inner loop (test-time training, with truncated BPTT for stability):
  ...
  - for chunk i = 0 .. n_chunks-1:
        y_i      = q_i · W_{i-1}.T                    (output for this chunk)
...
ttt_W_init and ttt_inner_lr route to Adam (scalar_lr) via CONTROL_TENSOR_NAME_PATTERNS;
the projections route to Muon (matrix_lr).

TTT is inserted at PRE-PR encoder blocks only (layers 0..psl-1). At PR-mode
blocks (layers psl..N-1) the existing two-lane PR path is unchanged. The TTT
sublayer is added between attention and MLP:
    h ← h + s_attn · attn(norm(h))
    h ← h + s_ttt  · ttt(norm(h))          (NEW, only if this block has TTT enabled)
    h ← h + s_mlp  · mlp(norm(h))
The out_proj of TTT is zero-initialised so TTT is transparent at step 0.

Env vars:
  TTT_LAYERS         (default "1,2,3")  — comma-separated indices of pre-PR layers with TTT
  TTT_STATE_DIM      (default 64)       — hidden state dimension d_state
  TTT_CHUNK_SIZE     (default 64)       — chunk size for the inner scan (T must divide)
  TTT_INNER_LR_INIT  (default 1.0)      — initial value for the learned inner_lr scalar
"""
...
class Hyperparameters:
    ...
    # ---- Test-Time Training (TTT-Linear, Sun et al. 2024) ----
    # Empty TTT_LAYERS disables TTT entirely (the file degenerates to the baseline).
    ttt_layers = sorted({int(s) for s in os.environ.get("TTT_LAYERS", "").split(",") if s.strip()})
    ttt_state_dim = int(os.environ.get("TTT_STATE_DIM", "64"))
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", "64"))
    ttt_inner_lr_init = float(os.environ.get("TTT_INNER_LR_INIT", "1.0"))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "ttt_W_init,ttt_inner_lr,ttt_scale",
    ).split(",")
    if pattern
)
...
class TTTLinear(nn.Module):
    """Test-Time Training Linear layer (Sun et al. 2024) — simplified for pre-PR blocks.

    Maintains a per-sequence hidden state W of shape (d_state, d_state) that is updated
    online via gradient descent on the self-supervised reconstruction objective:
        loss(W; k, v) = ||W·k - v||²
    on each chunk of size C tokens. The output for a chunk is W·q, applied with W
    *as of the previous chunk* (causal). The W state is reset at the start of every
    forward pass (per-batch reset, no leakage across mini-batches).

    Outer-loop learnables (trained by the main optimizer):
      - q_proj, k_proj, v_proj, out_proj  (CastedLinear, route to Muon at matrix_lr)
      - ttt_W_init                        (the learned init for W; routes to Adam)
      - ttt_inner_lr                      (the learned inner-loop step size; routes to Adam)

    The out_proj is zero-initialised so the layer is transparent at step 0 of outer
    training — exactly like the attention output gate.
    """
    ...
    def __init__(self, d_model: int, d_state: int = 64, chunk_size: int = 64,
        ...
        self.norm = RMSNorm()
        self.q_proj = CastedLinear(d_model, d_state, bias=False)
        self.k_proj = CastedLinear(d_model, d_state, bias=False)
        self.v_proj = CastedLinear(d_model, d_state, bias=False)
        self.out_proj = CastedLinear(d_state, d_model, bias=False)
        self.out_proj._zero_init = True
        self.ttt_W_init = nn.Parameter(torch.zeros(d_state, d_state, dtype=torch.float32))
        self.ttt_inner_lr = nn.Parameter(torch.tensor(float(inner_lr_init), dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        """x: [B, T, d_model]. Returns residual update of shape [B, T, d_model]."""
        B, T, _ = x.shape
        C = self.chunk_size
        if T % C != 0:
            raise ValueError(f"TTT chunk_size {C} must divide seq_len {T}")
        n_chunks = T // C

        x_norm = self.norm(x)
        q = self.q_proj(x_norm).view(B, n_chunks, C, self.d_state)
        k = self.k_proj(x_norm).view(B, n_chunks, C, self.d_state)
        v = self.v_proj(x_norm).view(B, n_chunks, C, self.d_state)

        W = self.ttt_W_init.to(dtype=x.dtype).unsqueeze(0).expand(B, -1, -1).contiguous()
        inner_lr = self.ttt_inner_lr.to(dtype=x.dtype)
        inv_C = 1.0 / float(C)
        ...
        for i in range(n_chunks):
            ...
            # Output: y_i = q_i @ W_{i-1}^T  (causal: uses W from before this chunk's update)
            yi = qi @ W.transpose(-1, -2)
            outputs.append(yi)

            # Inner step: gradient of ||W·k - v||² wrt W is 2 · (W·k - v) · k^T (per-token mean).
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 ttt_enabled: bool = False, ttt_state_dim: int = 64,
                 ttt_chunk_size: int = 64, ttt_inner_lr_init: float = 1.0):
        super().__init__()
        ...
        # TTT sublayer — only allocated when enabled, only used in pre-PR (single-lane) blocks.
        self.ttt_enabled = ttt_enabled
        if ttt_enabled:
            self.ttt = TTTLinear(dim, d_state=ttt_state_dim,
                                 chunk_size=ttt_chunk_size,
                                 inner_lr_init=ttt_inner_lr_init)
            self.ttt_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        if self.ttt_enabled:
            x = x + self.ttt_scale.to(dtype=x.dtype)[None, None, :] * self.ttt(x)
...
class GPT(nn.Module):
    def __init__(
        ...
        # TTT config — frozenset of layer indices that get a TTT sublayer.
        # Constraint: TTT layers must be PRE-PR (single-lane) — i.e., index < parallel_start_layer.
        # We don't currently support TTT in PR-mode blocks (would require two-lane TTT handling).
        self.ttt_layers = frozenset(ttt_layers or [])
        self.ttt_state_dim = ttt_state_dim
        ...
        def _layer_ttt(i: int) -> bool:
            if i not in self.ttt_layers:
                return False
            # Sanity check: TTT only supported on pre-PR (single-lane) layers.
            if 0 <= parallel_start_layer < num_layers and i >= parallel_start_layer:
                raise ValueError(
                    f"TTT requested at layer {i} but parallel_start_layer={parallel_start_layer}. "
                    f"TTT is only supported at pre-PR layers (index < parallel_start_layer)."
                )
            return True
        self.blocks = nn.ModuleList([
            Block(
                ...
                ttt_enabled=_layer_ttt(i), ttt_state_dim=ttt_state_dim,
                ttt_chunk_size=ttt_chunk_size, ttt_inner_lr_init=ttt_inner_lr_init,
            )
...
def run_one(
    ...
    base_model = GPT(
        ...
        ttt_state_dim=args.ttt_state_dim,
        ttt_chunk_size=args.ttt_chunk_size,
        ttt_inner_lr_init=args.ttt_inner_lr_init,
    ).to(device).bfloat16()
    ...
    n_ttt_params = sum(
        ...
        if ".ttt." in n or n.endswith(".ttt_scale")
    )
    log0(f"model_params:{n_params}  parallel_params:{n_parallel_params}  "
         f"gate_params:{n_gate_params}  ttt_params:{n_ttt_params}  "
         f"ttt_layers:{sorted(args.ttt_layers)}  ttt_state_dim:{args.ttt_state_dim}  "
         f"parallel_start_layer:{parallel_start_layer}  "
    ...
    if master_process:
        ...
        run_config.update({
            ...
            # TTT config
            "ttt_layers":         sorted(args.ttt_layers),
            "ttt_layers_count":   len(args.ttt_layers),
            "ttt_state_dim":      args.ttt_state_dim,
            "ttt_chunk_size":     args.ttt_chunk_size,
            "ttt_inner_lr_init":  args.ttt_inner_lr_init,
            "ttt_params":         n_ttt_params,
            # Quantization config
    ...
    while True:
        ...
        if should_log:
            ...
            if master_process:
                ...
                # Log per-layer TTT diagnostics (when enabled).
                # ttt_inner_lr tracks the learned inner step size; ttt_W_init_norm tracks
                # how much the learned init has drifted from zero (the "transparent" start).
                if base_model.ttt_layers:
                    inner_lrs   = [p.detach().float().item()
                                   for n, p in base_model.named_parameters() if n.endswith("ttt_inner_lr")]
                    w_init_norms = [p.detach().float().norm().item()
                                    for n, p in base_model.named_parameters() if n.endswith("ttt_W_init")]
                    ttt_scales   = [p.detach().float().abs().mean().item()
                                    for n, p in base_model.named_parameters() if n.endswith(".ttt_scale")]
                    if inner_lrs:
                        log_dict["ttt_inner_lr_mean"]     = float(np.mean(inner_lrs))
                        log_dict["ttt_inner_lr_max"]      = float(np.max(np.abs(inner_lrs)))
                    if w_init_norms:
                        log_dict["ttt_W_init_norm_mean"] = float(np.mean(w_init_norms))
                    if ttt_scales:
                        log_dict["ttt_scale_mean_abs"]   = float(np.mean(ttt_scales))
