...
USE_GPTQ = int(os.environ.get("USE_GPTQ", 0))
USE_GPTQ_LQER = int(os.environ.get("USE_GPTQ_LQER", 0))
...
QUANT_ASYMMETRIC = int(os.environ.get("QUANT_ASYMMETRIC", 1))
COMPRESSION_METHOD = os.environ.get("COMPRESSION_METHOD", "zlib")  # "zlib" or "brotli"
BROTLI_QUALITY = int(os.environ.get("BROTLI_QUALITY", 11))


def byte_shuffle(data: bytes, element_width: int = 1) -> bytes:
    """Rearrange bytes to group corresponding byte positions together.

    For multi-byte values (e.g. fp16 scales with width=2), this groups all
    high bytes together and all low bytes together, dramatically increasing
    redundancy for the compressor.

    For single-byte data (int8 weights), we use width=1 which is a no-op.
    The benefit comes from applying it to the entire serialized blob where
    fp16/fp32 values are interleaved with int8 data.
    """
    if element_width <= 1:
        return data
    arr = np.frombuffer(data, dtype=np.uint8)
    # Trim to multiple of element_width
    usable = len(arr) // element_width * element_width
    remainder = arr[usable:]
    shuffled = arr[:usable].reshape(-1, element_width).T.tobytes()
    return shuffled + remainder.tobytes()


def byte_unshuffle(data: bytes, element_width: int = 1, original_length: int = None) -> bytes:
    ...
    arr = np.frombuffer(data, dtype=np.uint8)
    usable = original_length // element_width * element_width
    num_elements = usable // element_width
    shuffled_part = arr[:usable]
    remainder = arr[usable:]
    unshuffled = shuffled_part.reshape(element_width, num_elements).T.tobytes()
    return unshuffled + remainder.tobytes()
...
def compress_blob(data: bytes) -> bytes:
    ...
    if COMPRESSION_METHOD == "brotli":
        ...
        # Apply byte-shuffle with width=2 (most tensors are int8 + fp16 scales)
        shuffled = byte_shuffle(data, element_width=2)
        return brotli.compress(shuffled, quality=BROTLI_QUALITY)
    else:
        return zlib.compress(data, level=9)


def decompress_blob(data: bytes, original_length: int = None) -> bytes:
    """Decompress using the configured method."""
    if COMPRESSION_METHOD == "brotli":
        if not HAS_BROTLI:
            raise RuntimeError("Brotli requested but not installed. Run: pip install brotli")
        decompressed = brotli.decompress(data)
        return byte_unshuffle(decompressed, element_width=2, original_length=original_length)
    else:
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        if master_process:
            ...
            run_config["embed_quant_bits"] = EMBED_QUANT_BITS
            run_config["use_gptq"] = USE_GPTQ
            run_config["use_gptq_lqer"] = USE_GPTQ_LQER
            run_config["lqer_rank"] = LQER_RANK
            run_config["lqer_top_k"] = LQER_TOP_K
            run_config["quant_group_size"] = QUANT_GROUP_SIZE
            run_config["quant_asymmetric"] = QUANT_ASYMMETRIC
            run_config["compression_method"] = COMPRESSION_METHOD
            run_config["brotli_quality"] = BROTLI_QUALITY if COMPRESSION_METHOD == "brotli" else None
            wandb.init(
        ...
        ext = "ptz" if COMPRESSION_METHOD == "zlib" else "ptb"
        model_int8_path = f"final_model_seed{seed}.int8.{ext}"
        ...
        if USE_GPTQ_LQER:
            quant_obj, quant_stats = gptq_lqer_quantize_state_dict(
                ...
                matrix_bits=MATRIX_QUANT_BITS,
                embed_bits=EMBED_QUANT_BITS,
                lqer_rank=LQER_RANK,
                lqer_top_k=LQER_TOP_K,
                group_size=QUANT_GROUP_SIZE,
                asymmetric=bool(QUANT_ASYMMETRIC),
                device=device,
            )
            use_custom_dequant = True
        elif USE_GPTQ:
            from gptq import gptq_quantize_state_dict
            quant_obj, quant_stats = gptq_quantize_state_dict(
                model=base_model,
                calibration_tokens=val_tokens,
                quant_bits=MATRIX_QUANT_BITS,
                device=device,
            )
            use_custom_dequant = False
        else:
        ...
        if master_process:
            ...
            log0(
                f"Serialized model ({COMPRESSION_METHOD}): {quant_file_bytes} bytes "
        ...
        if use_custom_dequant:
            base_model.load_state_dict(dequantize_gptq_lqer(quant_state), strict=True)
        else:
