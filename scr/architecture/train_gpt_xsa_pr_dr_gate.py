"""
XSA × PR × DR × Gate (quad stack) — additivity test of XSA on top of the project headline.

Adds XSA (cross-self-attention, value-orthogonalization) from the friend's
train_gpt_XSA.py to the abl8b project headline (PR psl=4 + DR recur=[2,3,4,5]
target=both + Gate src=proj w=12 at INT8).

XSA is parameter-free: after SDPA produces y = [B, H, T, D], it projects y onto
the L2-normalised value direction v̂ and subtracts that component, leaving the
output orthogonal to its own value vector. Forces each head to produce signal
beyond "just copy v" (which the self-attention term tends to do).

  v_hat        = normalize(v_full, dim=-1)              # [B, H, T, D]
  proj_scalar  = (y * v_hat).sum(dim=-1, keepdim=True)  # [B, H, T, 1]
  y            = y - proj_scalar * v_hat                # orthogonal component

In the gate-enabled attention, the forward-pass order is:
  SDPA  →  XSA (optional)  →  transpose [B,H,T,D]→[B,T,H,D]  →  gate (optional)  →  reshape  →  proj

Defaults: PR psl=4 + DR [2,3,4,5] both + Gate w=12 + XSA on. Setting USE_XSA=0
reproduces the abl8b headline (1.2927 ± 0.0007) exactly — a bit-identical control.

Sweep axis is SEEDS (default 1337,42,123); a 3-seed run reproduces the headline-
analog with this single extra feature, letting us see if the gain is additive.
...
class Hyperparameters:
    ...
    base_run_id = os.environ.get("RUN_ID", "xsa_pr_dr_gate")
    ...
    # ---- XSA (cross-self-attention, value-orthogonalization) ----
    # Parameter-free attention modification ported from train_gpt_XSA.py.
    # After SDPA, projects y onto v̂ and subtracts → leaves y orthogonal to its
    # own value direction. USE_XSA=0 reproduces the abl8b headline exactly
    # (bit-identical control). USE_XSA=1 enables it (the additivity test).
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        # XSA — parameter-free value-orthogonalization (ported from train_gpt_XSA.py).
        # Applied AFTER SDPA and BEFORE the gate (if any). Does NOT add params.
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # ---- XSA: orthogonalize y against its own L2-normalised value direction ----
        # Lives here (before transpose) so v is still [B, H_kv, T, D] like in
        # train_gpt_XSA.py. For GQA (num_kv_heads != num_heads) we expand v back
        # to per-query-head before normalisation, mirroring how SDPA broadcasts KV.
        if self.use_xsa:
            if self.num_kv_heads != self.num_heads:
                groups = self.num_heads // self.num_kv_heads
                v_full = v.repeat_interleave(groups, dim=1)  # [B, H, T, D]
            else:
                v_full = v
            v_hat = F.normalize(v_full, dim=-1)               # [B, H, T, D]
            proj_scalar = (y * v_hat).sum(dim=-1, keepdim=True)  # [B, H, T, 1]
            y = y - proj_scalar * v_hat                       # remove self-value component
        # ----------------------------------------------------------------------------
...
class GPT(nn.Module):
    def __init__(
        ...
        # XSA flag — passed unconditionally to every Block (it's parameter-free, so
        # it doesn't matter at construction time; only the forward branches on it).
        self.use_xsa = use_xsa
...
def run_one(
    ...
    xsa_tag = "xsaON" if use_xsa else "xsaOFF"
    run_id = f"{args.base_run_id}_{psl_tag}_recur{layers_tag}_{args.recur_target}_{gate_tag}_{xsa_tag}_seed{seed}"
    log0(f"\n{'='*100}")
    log0(f"XSA × PR × DR × Gate  psl={parallel_start_layer}  asym={parallel_asym_init}  "
    ...
    if master_process:
        ...
        wandb.init(
            ...
            name=run_id,
            config=run_config,
            tags=["xsa_pr_dr_gate", psl_tag, f"target_{args.recur_target}",
                  gate_tag, f"width_{gate_width}", xsa_tag],
        )
    ...
    # Serialisation + quantised final eval
    model_int8_path = f"final_model_{run_id}.int8.ptz"
    ...
    if master_process:
        ...
        log0(f"Serialised int8+zlib: {quant_file_bytes} bytes  total: {quant_file_bytes + code_bytes} bytes")
...
def main() -> None:
    ...
    if master_process:
        print("XSA × Parallel Residuals × Depth Recurrence × Attention Gate")
