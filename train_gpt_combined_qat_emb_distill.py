"""
COMBINED + POST-TRAINING SELF-DISTILLATION: PR × DR × Gate + mixed INT6/INT8 + QAT
                                             + a short self-distillation phase between
                                             training and quantisation.

Identical to train_gpt_combined_qat_emb.py through the end of the main 5000-iteration
training loop. THEN, before quantisation, a NEW phase runs:

  1. Snapshot the just-trained weights as the "teacher" (frozen, fp32, no QAT).
  2. Initialise the "student" as an in-place copy of the same weights, with QAT
     fake-quantisation active in its forward pass.
  3. For K distillation steps on training-data batches:
        - Teacher logits (no_grad, QAT off) — the clean target distribution.
        - Student logits  (with grad, QAT on) — what we'd actually deploy.
        - Loss = KL(student || teacher) at temperature T.
        - Optimizer.step() on student weights.
  4. The student is now adjusted so that, when quantised, it produces logits close
     to the un-quantised teacher's logits.
  5. Quantise the student through the existing pipeline (unchanged) and save.

This is genuinely novel for parameter-golf: every entry on the leaderboard does
straight QAT or PTQ, with no post-training self-distillation phase. The mechanism
is strictly stronger than QAT alone — QAT teaches "be robust to rounding noise",
distillation teaches "be robust *and* exactly match the un-quantised teacher".

The quantisation function itself is COMPLETELY UNCHANGED — distillation is purely
a post-training fine-tuning step.

Env vars:
  DISTILL_ENABLED      (default 1)        — set to 0 to skip the distillation phase
  DISTILL_STEPS        (default 200)      — number of distillation gradient steps
  DISTILL_LR           (default 1e-4)     — learning rate for distillation
  DISTILL_TEMPERATURE  (default 1.0)      — temperature T for the KL distillation loss
  DISTILL_OPTIMIZER    (default "adamw")  — "adamw" or "sgd"
"""
...
class Hyperparameters:
    ...
    # ---- Post-training self-distillation (between training and quantisation) ----
    # The teacher is the just-trained model (QAT off in its forward pass).
    # The student starts as a copy and is fine-tuned with KL loss against the
    # teacher's logits, with QAT fake-quantisation ACTIVE in the student's forward.
    # This pushes the student toward weights that, when quantised, still match
    # the teacher's predictions. The quantisation function itself is unchanged.
    distill_enabled = bool(int(os.environ.get("DISTILL_ENABLED", "1")))
    distill_steps = int(os.environ.get("DISTILL_STEPS", "200"))
    distill_lr = float(os.environ.get("DISTILL_LR", "1e-4"))
    distill_temperature = float(os.environ.get("DISTILL_TEMPERATURE", "1.0"))
    distill_optimizer = os.environ.get("DISTILL_OPTIMIZER", "adamw").lower()
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor | None = None,
        ...
        if return_logits:
            return logits
        assert target_ids is not None, "target_ids must be provided when return_logits=False"
...
def run_one(
    ...
    # ---- Post-training self-distillation phase ----
    # Snapshot the trained model as the TEACHER (frozen, QAT off in its forward).
    # The base_model is the STUDENT (its weights will be updated by distillation,
    # with QAT fake-quant active in its forward).
    # After K steps, the student's weights are what gets quantised below.
    if args.distill_enabled and args.distill_steps > 0:
        import copy as _copy
        log0(f"[DISTILL] starting post-training self-distillation  "
             f"steps={args.distill_steps}  lr={args.distill_lr}  "
             f"T={args.distill_temperature}  optimizer={args.distill_optimizer}")
        torch.cuda.synchronize()
        t_distill = time.perf_counter()

        # Snapshot teacher — deepcopy of base_model (weights only; same architecture).
        teacher = _copy.deepcopy(base_model).eval()
        for p in teacher.parameters():
            p.requires_grad_(False)

        # Student optimizer over all student parameters.
        if args.distill_optimizer == "adamw":
            distill_opt = torch.optim.AdamW(base_model.parameters(),
                                            lr=args.distill_lr,
                                            betas=(0.9, 0.95),
                                            weight_decay=0.0, fused=True)
        elif args.distill_optimizer == "sgd":
            distill_opt = torch.optim.SGD(base_model.parameters(),
                                          lr=args.distill_lr, momentum=0.9)
        else:
            raise ValueError(f"Unknown DISTILL_OPTIMIZER {args.distill_optimizer!r}")

        T = args.distill_temperature
        student = base_model
        student.train()
        # Keep QAT enabled in student's forward — this is the whole point.
        prev_qat = CastedLinear._qat_enabled
        CastedLinear._qat_enabled = True

        distill_losses: list[float] = []
        for d_step in range(args.distill_steps):
            x, _ = train_loader.next_batch(args.train_batch_tokens,
                                           args.train_seq_len, grad_accum_steps)
            # Teacher forward: QAT OFF, no_grad. Use eval()-mode forward (no fake-quant).
            CastedLinear._qat_enabled = False
            with torch.no_grad():
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    teacher_logits = teacher(x, return_logits=True)
            # Student forward: QAT ON, with grad.
            CastedLinear._qat_enabled = True
            distill_opt.zero_grad(set_to_none=True)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                student_logits = student(x, return_logits=True)
            # KL(student || teacher) at temperature T, scaled by T^2 (standard practice).
            log_student = F.log_softmax(student_logits.float() / T, dim=-1)
            soft_teacher = F.softmax(teacher_logits.float() / T, dim=-1)
            loss = F.kl_div(log_student, soft_teacher, reduction="batchmean") * (T * T)
            loss.backward()
            distill_opt.step()
            distill_losses.append(float(loss.item()))
            if d_step < 5 or (d_step + 1) % 20 == 0 or d_step == args.distill_steps - 1:
                recent = distill_losses[-min(10, len(distill_losses)):]
                log0(f"[DISTILL] step {d_step+1:4d}/{args.distill_steps}  "
                     f"kl_loss={float(loss.item()):.4f}  recent_mean={sum(recent)/len(recent):.4f}")

        # Restore QAT flag to whatever it was (always True at this point in practice).
        CastedLinear._qat_enabled = prev_qat
        student.eval()  # back to eval-like for quantisation
        del teacher
        torch.cuda.synchronize()
        log0(f"[DISTILL] done in {time.perf_counter()-t_distill:.1f}s  "
             f"final_kl={distill_losses[-1]:.4f}  mean_last10={sum(distill_losses[-10:])/min(10,len(distill_losses)):.4f}")

        if master_process:
            wandb.log({
                "distill_steps_done": len(distill_losses),
                "distill_final_kl": distill_losses[-1] if distill_losses else None,
                "distill_mean_kl_last10": (sum(distill_losses[-10:]) / min(10, len(distill_losses)))
                                          if distill_losses else None,
                "distill_lr": args.distill_lr,
                "distill_temperature": T,
                "distill_optimizer": args.distill_optimizer,
            })
