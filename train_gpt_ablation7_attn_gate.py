...
Ports the per-head attention output gate from openai/parameter-golf PR #1667
("SmearGate + Attention Output Gate + Score-First TTT", 2026-04-16).

Per-layer mechanism (only the attention sub-layer changes; MLP is untouched):

    y = SDPA(q, k, v)                                  # [B, T, H, D]
    g = 2 * sigmoid( W_gate @ src[..., :gate_width] )  # [B, T, H]   per-head, per-token
    y = y * g[..., None]                               # broadcast over head_dim
    out = proj(reshape(y))

  - W_gate is `CastedLinear(gate_width, num_heads, bias=False)`, ZERO-initialised
    so that at step 0:  g = 2*sigmoid(0) = 1.0  →  the gate is the identity and
    the network is bit-identical to baseline. Anything it learns is pure upside.
  - `src` is one of:
        GATE_ATTN_SRC=proj  →  the raw block input `x`           (leaderboard default)
        GATE_ATTN_SRC=q     →  the Q-projection output, pre-RoPE
    Both tap the first `gate_width` channels only — tiny conditioning slice.
  - W_gate is registered under CONTROL_TENSOR_NAME_PATTERNS ("attn_gate_proj")
    so it uses the scalar LR group, not Muon. Same convention as the leaderboard.

Defaults reproduce the PR #1667 config:
    GATE_ATTN_OUT=1, GATE_ATTN_SRC=proj, GATE_WIDTH=12   (≈ 96 params/layer, 8*12)

Sweep axis is just SEEDS (default 1337); other knobs are scalar env vars.
The plan, mirroring how PR (abl3 → abl4) was validated:
    abl7   — single-seed sweep over (gate_attn_src × gate_width) + disabled control
    abl7b  — multi-seed validation of the winner (3 seeds)
"""
...
class Hyperparameters:
    ...
    base_run_id = os.environ.get("RUN_ID", "ablation7_attn_gate")

    # Sweep axis: seeds. Other knobs are scalar.
    seeds = [int(s) for s in os.environ.get("SEEDS", "1337").split(",")]

    # Attention output gate (per-head, sigmoid-gated, applied between SDPA and proj).
    # GATE_ATTN_OUT=0 → bit-identical to baseline (no extra params constructed).
    # GATE_ATTN_OUT=1 → constructs a CastedLinear(gate_width, num_heads, bias=False)
    #                   per layer; zero-init so g = 2*sigmoid(0) = 1.0 at step 0.
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        # The leaderboard PR #1667 routes it to scalar_lr (not Muon) via this pattern.
        "attn_scale,mlp_scale,resid_mix,q_gain,skip_weight,skip_weights,attn_gate_proj",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    """Baseline attention + optional per-head sigmoid output gate.

    Mirrors openai/parameter-golf PR #1667. With gate_attn_out=False the forward
    pass is bit-identical to the baseline (and no extra parameters are constructed).
    """
    def __init__(
        self,
        dim: int,
...
class GPT(nn.Module):
    """Baseline U-Net GPT with an optional per-layer attention output gate.

    Architecture is identical to the baseline: half-encoder, half-decoder with
    learned per-channel skip weights. The only change is each Block's attention
    optionally carries a sigmoid output gate (see CausalSelfAttention).
    """
    def __init__(
        self,
        vocab_size: int,
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # ---- Decoder (U-Net skips popped in reverse) ----
...
def run_one(
    ...
    """One full training+eval cycle for a single seed of the attn-gate config."""
    ...
    run_id = f"{args.base_run_id}_{gate_tag}_seed{seed}"
    log0(f"\n{'='*100}")
    log0(f"ABLATION 7 (Attn Output Gate)  enabled={gate_attn_out}  "
         f"src={gate_attn_src}  width={gate_width}  seed={seed}")
    ...
    # Optimizer groups — Muon for matrix params, Adam for scalars + gate Linear.
    ...
    log0(f"model_params:{n_params}  gate_params:{n_gate_params}  "
    ...
    if master_process:
        ...
        wandb.init(
            ...
            name=run_id,
            config=run_config,
            tags=["ablation7_attn_gate", gate_tag, f"src_{gate_attn_src}", f"width_{gate_width}"],
        )
    ...
    while True:
        ...
        if should_log:
            ...
            if master_process:
                ...
                # Gate weight has shape [num_heads, gate_width]; mean(|W|) tracks how
                # far the gate has moved away from its zero-init (transparent) point.
...
def main() -> None:
    ...
    # Single specialisation path (gate enabled/disabled is a structural choice, not
    # a runtime branch), but kept generous for safety.
    torch._dynamo.config.recompile_limit = 64

    # Inherit the inductor workaround from earlier ablations — cheap insurance against
    # the FusedMixOrderReductions assertion that bit us in abl3/abl4. Harmless if not needed.
    ...
    if master_process:
        print("Ablation 7 — Per-head sigmoid Attention Output Gate (PR #1667 port)")
