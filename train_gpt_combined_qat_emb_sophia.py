"""
COMBINED + SOPHIA-IN-QAT: PR × DR × Attention Gate + Mixed int6/int8 + QAT
                          + SophiaG replaces Muon for matrix params during the QAT phase.

Identical to train_gpt_combined_qat_emb.py with one optimizer change:
  - For iters 0 .. qat_start_step the matrix-valued Transformer params are optimised
    by Muon (the baseline).
  - At QAT activation, Muon is REPLACED by a simplified SophiaG optimizer for the
    same param group. SophiaG is a second-order-ish optimizer that uses element-
    wise clipping of m / h (first-moment / diagonal-Hessian proxy) — argued
    to converge to flatter minima than first-order methods, which should improve
    robustness to QAT/post-training rounding noise.

This variant uses the **gradient-squared diagonal Fisher proxy** as the Hessian
estimator (the Bartlett identity says this is proportional to the diagonal Fisher,
which approximates the GNB-Hessian estimate from the Sophia paper). It adds zero
extra forward/backward passes compared to a normal training step.

Env vars:
  SOPHIA_LR              (default 3e-4) — Sophia lr for matrix params (used after QAT)
  SOPHIA_RHO             (default 0.04) — Sophia clip threshold for m/h
  SOPHIA_BETA1           (default 0.965)
  SOPHIA_BETA2           (default 0.99)
"""
...
class Hyperparameters:
    ...
    # ---- Sophia (second-order optimizer used after QAT activation) ----
    # SophiaG replaces Muon for matrix-valued params once QAT activates.
    # Hyperparameters follow the Sophia paper (Liu et al. 2023) defaults at small scale.
    sophia_lr = float(os.environ.get("SOPHIA_LR", "3e-4"))
    sophia_rho = float(os.environ.get("SOPHIA_RHO", "0.04"))
    sophia_beta1 = float(os.environ.get("SOPHIA_BETA1", "0.965"))
    sophia_beta2 = float(os.environ.get("SOPHIA_BETA2", "0.99"))
    sophia_eps = float(os.environ.get("SOPHIA_EPS", "1e-12"))
...
# -----------------------------
# SOPHIA-G (simplified, grad²-as-Fisher proxy)
# -----------------------------

class SophiaG(torch.optim.Optimizer):
    """Simplified SophiaG optimizer.

    Implements the Sophia update:
        m_t = β₁ · m_{t-1} + (1 - β₁) · g_t
        h_t = β₂ · h_{t-1} + (1 - β₂) · g_t²       (diagonal Fisher proxy via Bartlett)
        θ_t = θ_{t-1} - η · clip(m_t / max(ρ · h_t, ε), -1, 1)

    The clip(·, -1, 1) bounds each per-parameter update to a step size of η, regardless
    of the curvature estimate. This is the central mechanism that promotes flatter
    minima: parameters with low curvature get a full step; parameters in high-curvature
    directions are heavily damped.

    Unlike the original Sophia-G, we use g² directly (rather than the GNB sampled-label
    estimate) — this is the diagonal Fisher information, which is proportional to the
    GNB estimate by Bartlett's identity. The simplification avoids modifying the GPT
    forward pass and keeps the training loop identical to the baseline.
    """

    def __init__(self, params, lr: float, betas: tuple[float, float] = (0.965, 0.99),
                 rho: float = 0.04, eps: float = 1e-12):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # 1st-moment: standard EMA of the gradient.
                m.mul_(beta1).add_(g32, alpha=1.0 - beta1)
                # Diagonal Hessian proxy: EMA of g² (Bartlett's diagonal-Fisher identity).
                h.mul_(beta2).addcmul_(g32, g32, value=1.0 - beta2)

                # Sophia step: clip( m / max(ρ·h, ε), -1, 1 ).
                denom = (h * rho).clamp_min(eps)
                ratio = (m / denom).clamp_(-1.0, 1.0)
                step_tensor = ratio.to(dtype=p.dtype)
                p.add_(step_tensor, alpha=-lr)
                total_update_sqnorm += float((step_tensor.float() * lr).pow(2).sum().item())

        self.last_update_norm = total_update_sqnorm ** 0.5
        return loss
...
def run_one(
    ...
    # from step 0 instead of from qat_start_step, AND preventing the Muon → Sophia swap
    # from firing for seeds 2..N (since the swap is guarded by `not _qat_enabled`).
    ...
    # Sophia optimizer for matrix params — built alongside Muon, used only after QAT activation.
    optimizer_sophia = SophiaG(matrix_params, lr=args.sophia_lr,
                               betas=(args.sophia_beta1, args.sophia_beta2),
                               rho=args.sophia_rho, eps=args.sophia_eps)
    for group in optimizer_sophia.param_groups:
        group["base_lr"] = args.sophia_lr
    # Pointer to whichever optimizer is currently driving the matrix params (Muon → Sophia at QAT).
    ...
    while True:
        ...
        if args.qat_enabled and not CastedLinear._qat_enabled:
            ...
            if step >= qat_start_step:
                ...
                model = DDP(compiled_model, device_ids=[local_rank],
                            broadcast_buffers=False) if distributed else compiled_model
                # SWAP: replace Muon with Sophia for matrix params from this step onward.
                muon_idx = optimizers.index(optimizer_muon)
                optimizers[muon_idx] = optimizer_sophia
                matrix_opt = optimizer_sophia
                log0(f"[SOPHIA] Replaced Muon → SophiaG for matrix params at step {step}  "
                     f"(lr={args.sophia_lr}  rho={args.sophia_rho}  "
                     f"betas=({args.sophia_beta1},{args.sophia_beta2}))")
                if master_process:
                    wandb.log({"qat_activated": 1, "sophia_active": 1,
                               "tokens_seen": step * args.train_batch_tokens}, step=step)
        ...
        if should_log:
            log0(f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                 ...
                 f"matrix_update_norm:{matrix_opt.last_update_norm:.4f} "
                 f"matrix_opt:{type(matrix_opt).__name__}")
            if master_process:
                log_dict = {
                    ...
                    "sophia_update_norm": optimizer_sophia.last_update_norm,
                    "matrix_update_norm": matrix_opt.last_update_norm,
                    "matrix_opt_is_sophia": int(matrix_opt is optimizer_sophia),
