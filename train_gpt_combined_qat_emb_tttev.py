"""
COMBINED + SCORE-FIRST TTT-EVAL: PR × DR × Gate + mixed INT6/INT8 + QAT
                                  + test-time training during validation.

Identical to train_gpt_combined_qat_emb.py through training and final-quantised eval.
After the standard `final_val_bpb` is recorded, a SECOND evaluation runs the validation
set under a "score-first TTT" protocol (the variant used by the parameter-golf
leaderboard's top entries):

  For each batch of validation tokens:
    1. SCORE  — forward pass under no_grad; this loss is what counts toward BPB.
    2. UPDATE — forward + backward + optimizer.step() on the same batch.
    3. Move on to the next batch (causally: it is scored under the updated weights).

Causality is preserved because each batch is scored *before* being used as training
data. The validation set is processed sequentially, so each batch's score only sees
weights trained on earlier batches.

The TTT-eval BPB is reported as `final_val_bpb_tttev` (separate from `final_val_bpb`).
The base 16 MB artifact is unchanged — TTT-eval operates on the dequantised weights
loaded back from the artifact, and the gradient updates are transient (never saved).

Env vars:
  TTT_EVAL_ENABLED   (default 1)        — set to 0 to skip the TTT-eval pass
  TTT_EVAL_LR        (default 1e-4)     — learning rate for the eval-time optimizer
  TTT_EVAL_STEPS     (default 1)        — number of gradient steps per validation batch
  TTT_EVAL_OPTIMIZER (default "adamw")  — "adamw" or "sgd"
  TTT_EVAL_RESET     (default 1)        — snapshot/restore the dequantised state around the call
"""
...
def eval_val_with_score_first_ttt(
    ...
    """Score-first TTT-eval (Sun 2020 style, the parameter-golf leaderboard variant).
    ...
    # Build the optimizer over ALL trainable parameters of the loaded model.
    # AdamW is the most defensive choice; SGD is included for ablation.
...
def run_one(
    ...
    # ---- Score-first TTT-eval (the "real" leaderboard-style TTT) ----
    # Operates on the already-dequantised weights from the artifact roundtrip above.
    # The score for each batch is computed BEFORE the optimizer touches that batch's
    # weights, so causality is preserved. The base 16 MB artifact is untouched.
    if args.ttt_eval_enabled:
        # Snapshot the dequantised state so the live weights are unchanged after
        # TTT-eval (useful if the same `run_one` later runs more diagnostics).
        if args.ttt_eval_reset:
            ttt_eval_backup = {k: v.detach().clone() for k, v in base_model.state_dict().items()}

        log0(f"[TTT-EVAL] starting score-first TTT-eval  "
             f"lr={args.ttt_eval_lr}  steps={args.ttt_eval_steps}  optimizer={args.ttt_eval_optimizer}")
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        tttev_loss, tttev_bpb = eval_val_with_score_first_ttt(
        ...
        torch.cuda.synchronize()
        log0(f"[TTT-EVAL] final_val_bpb_tttev:{tttev_bpb:.4f}  val_loss:{tttev_loss:.4f}  "
             f"Δ_vs_base:{tttev_bpb - q_val_bpb:+.4f}  "
             f"eval_time:{1000.0*(time.perf_counter()-t_ttt):.0f}ms")

        if master_process:
            wandb.log({
                "final_val_loss_tttev": tttev_loss,
                "final_val_bpb_tttev": tttev_bpb,
                "tttev_delta_vs_base": tttev_bpb - q_val_bpb,
                "tttev_lr": args.ttt_eval_lr,
                "tttev_steps_per_batch": args.ttt_eval_steps,
                "tttev_optimizer": args.ttt_eval_optimizer,
            })

        # Restore the dequantised snapshot so subsequent code sees pristine weights.
        if args.ttt_eval_reset:
