...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,q_gain,full_attn_res,attn_res_proj,attn_res_projs",
    ).split(",")
    if pattern
)
...
class FullAttentionResidual(nn.Module):
    def __init__(self, dim: int):
        ...
        self.last_num_prev_sources: int = 0

    def forward(self, prev_sources: list[Tensor], current_state: Tensor) -> Tensor:
        sources = prev_sources + [current_state]
        ...
        self.last_num_prev_sources = len(prev_sources)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.full_attn_res = nn.ModuleList([FullAttentionResidual(model_dim) for _ in range(num_layers)])
    ...
    def forward(
        ...
        for i, block in enumerate(self.blocks):
            x = self.full_attn_res[i](history, x)
            if return_diagnostics:
                hidden_norms[f"preblock_attnres_{i}"].append(tensor_l2_norm(x))
            x = block(x)
            history.append(x)
            if return_diagnostics:
                hidden_norms[f"block_{i}"].append(tensor_l2_norm(x))
...
@torch.no_grad()
def summarize_full_attention_stats(model: nn.Module) -> dict[str, float]:
    stats: dict[str, float] = {}
    modules = getattr(model, "full_attn_res", None)
    if modules is None:
        return stats
    ...
    for layer_idx, mod in enumerate(modules):
        ...
        for src_idx in range(s):
            ...
            stats[f"full_attn/layer_{layer_idx}/source_{src_idx}_mean_weight"] = value
            source_mass_sums[src_idx].append(value)

        entropy = -(w.clamp_min(1e-12) * w.clamp_min(1e-12).log()).sum(dim=0).mean()
        stats[f"full_attn/layer_{layer_idx}/entropy"] = float(entropy.item())
        mean_entropy.append(float(entropy.item()))

        current_mass = w[-1].mean()
        previous_mass = w[:-1].sum(dim=0).mean() if s > 1 else torch.zeros((), device=w.device)
        stats[f"full_attn/layer_{layer_idx}/current_state_mass"] = float(current_mass.item())
        stats[f"full_attn/layer_{layer_idx}/previous_source_mass"] = float(previous_mass.item())
        mean_current_mass.append(float(current_mass.item()))
        mean_previous_mass.append(float(previous_mass.item()))

    if mean_entropy:
        stats["full_attn/entropy_mean_over_layers"] = float(sum(mean_entropy) / len(mean_entropy))
    if mean_current_mass:
        stats["full_attn/current_state_mass_mean_over_layers"] = float(sum(mean_current_mass) / len(mean_current_mass))
    if mean_previous_mass:
        stats["full_attn/previous_source_mass_mean_over_layers"] = float(sum(mean_previous_mass) / len(mean_previous_mass))

    for src_idx, vals in source_mass_sums.items():
        stats[f"full_attn/source_{src_idx}_mean_weight_over_layers"] = float(sum(vals) / len(vals))
...
def summarize_attnres_grad_norms(model: nn.Module) -> dict[str, float]:
    ...
    for layer_idx, mod in enumerate(getattr(model, "full_attn_res", [])):
        for pname, p in mod.named_parameters():
            if p.grad is None:
                continue
            gnorm = float(p.grad.detach().float().norm().item())
            stats[f"grad_norm/full_attn/layer_{layer_idx}/{pname}"] = gnorm
            total_sq += gnorm ** 2
            count += 1

    if count > 0:
        stats["grad_norm/full_attn/global"] = total_sq ** 0.5
        stats["grad_norm/full_attn/num_params_with_grad"] = float(count)

    return stats


@torch.no_grad()
def build_full_weight_snapshot_table(model: nn.Module, step: int):
    rows = []
    for layer_idx, mod in enumerate(getattr(model, "full_attn_res", [])):
        if mod.last_weights is None:
            continue
        w = mod.last_weights.float().mean(dim=(1, 2))
        num_prev = mod.last_num_prev_sources
        for src_idx in range(w.numel()):
            source_type = "current_state" if src_idx == w.numel() - 1 else ("embedding" if src_idx == 0 else "previous_block")
            rows.append([step, layer_idx, src_idx, source_type, float(w[src_idx].item()), num_prev])

    if not rows:
        return None

    return wandb.Table(
        columns=["step", "layer", "source_index", "source_type", "mean_weight", "num_previous_sources"],
        data=rows,
    )
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        for module in base_model.full_attn_res:
            scalar_params.extend(list(module.parameters()))
            ...
            if should_log_train:
                ...
                if master_process:
                    ...
                    if should_log_attn_diag:
                        ...
                        log_payload.update(summarize_full_attention_stats(base_model))
                    ...
                    if should_log_snapshot:
                        snapshot_table = build_full_weight_snapshot_table(base_model, step)
                        if snapshot_table is not None:
                            wandb.log({"full_attn/source_weight_snapshot": snapshot_table}, step=step)
