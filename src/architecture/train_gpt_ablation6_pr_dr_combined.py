...
Composes the two multi-seed-validated winners from the standalone ablations:
  - Parallel residuals (abl3/abl4): psl=4 with symmetric init → 1.3057 ± 0.0016
  - Depth recurrence  (abl5f):      recur_layers=[2,3,4,5] target=both → 1.3022 ± 0.0029

Default config: PARALLEL_START_LAYER=4, RECUR_LAYERS="2,3,4,5", RECUR_TARGET=both.

Behaviour per layer i:
  ...
  i >= parallel_start_layer  → parallel block (two lanes, learned routing)
...
Both recurrence modes match the abl5b/abl5f convention of NOT re-applying the input
mixing on the extra pass (the "target=both" winner). This keeps the composition
faithful to the standalone winners.

The sweep axis is just SEEDS (default 1337). Other knobs are scalar env vars,
not sweeps — this script validates the composition, it does not re-explore.
"""
...
class Hyperparameters:
    ...
    base_run_id = os.environ.get("RUN_ID", "ablation6_pr_dr_combined")
...
def run_one(
    ...
    """One full training+eval cycle for a single seed of the combined PR+DR config."""
    ...
    run_id = f"{args.base_run_id}_{psl_tag}_recur{layers_tag}_{args.recur_target}_seed{seed}"
    log0(f"\n{'='*100}")
    log0(f"ABLATION 6 (PR×DR)  psl={parallel_start_layer}  asym={parallel_asym_init}  "
         f"recur_layers={sorted(args.recur_layers)}  recur_target={args.recur_target}  "
         f"recur_times={args.recur_times}  seed={seed}")
    ...
    if master_process:
        ...
        wandb.init(
            ...
            name=run_id,
            config=run_config,
            tags=["ablation6_pr_dr_combined", psl_tag, f"target_{args.recur_target}"],
        )
...
def main() -> None:
    ...
    if master_process:
        print("Ablation 6 — Parallel Residuals × Depth Recurrence (combined)")
