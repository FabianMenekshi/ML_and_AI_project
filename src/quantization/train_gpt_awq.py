...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        if master_process:
            ...
            export_state = base_model.state_dict()
            awq_stats["param_count"] = sum(int(t.numel()) for t in export_state.values())
            awq_stats["num_float_tensors"] = int(awq_stats["num_awq_layers"])
            awq_stats["baseline_tensor_bytes"] = sum(tensor_nbytes(t) for t in export_state.values())
            model_config = {
                "vocab_size": args.vocab_size,
                "num_layers": args.num_layers,
                "model_dim": args.model_dim,
        ...
        if master_process:
            wandb.log({
                ...
                "quant_file_bytes": awq_stats["file_bytes"],
                "quant_compression_ratio": awq_stats["baseline_tensor_bytes"] / max(awq_stats["payload_bytes"], 1),
                "quant_param_count": awq_stats["param_count"],
                "quant_num_float_tensors": awq_stats["num_float_tensors"],
                "quant_baseline_bytes": awq_stats["baseline_tensor_bytes"],
                "quant_int8_payload_bytes": awq_stats["payload_bytes"],
            })
            wandb.finish()
