"""
COMBINED + WEIGHT EMA: PR × DR × Attention Gate (architecture) + Mixed int6/int8 + QAT
                       + EMA-of-weights, with the EMA weights used for final quantization.

Identical to train_gpt_combined_qat_emb.py with one addition:
  - A Polyak/EMA shadow copy of the model weights is maintained during training.
  - At export time, the EMA weights are loaded into the model and THOSE are quantized.
  - EMA weights typically sit in a flatter region of the loss landscape, which makes
    them more quantization-robust.

Env vars:
  EMA_DECAY            (default 0.999)   — EMA decay (per optimizer step)
  EMA_START_FRACTION   (default 0.0)     — fraction of training before EMA tracking starts
  EMA_USE_AT_EXPORT    (default 1)       — if 1, use EMA weights for quantization
"""
...
class Hyperparameters:
    ...
    # The EMA tracks a slow copy of every model parameter:
    #     ema_p = ema_decay * ema_p + (1 - ema_decay) * live_p   (every optimizer step)
    # At export time, the EMA weights replace the live weights before quantization.
    # Motivation: EMA weights lie in a flatter region of the loss landscape and are
    # typically more robust to quantization rounding noise.
    ema_decay = float(os.environ.get("EMA_DECAY", "0.999"))
    ema_start_fraction = float(os.environ.get("EMA_START_FRACTION", "0.0"))
    ema_use_at_export = bool(int(os.environ.get("EMA_USE_AT_EXPORT", "1")))
...
class WeightEMA:
    """Maintains an exponential moving average of all trainable model parameters.

    On every call to update(), each EMA parameter is updated as:
        ema = decay * ema + (1 - decay) * live
    The EMA tensors live on the same device as the model and are kept in fp32 for
    numerical stability (the marginal extra memory is small for ~17M params).

    swap_into / swap_out toggle between live and EMA weights so the quantization
    pipeline can operate on the EMA copy without permanent state changes.
    """
    ...
    @torch.no_grad()
    def swap_into(self, model: nn.Module) -> None:
        """Replace live weights with EMA weights, stashing the live copy for restore."""
        if self._backup:
            raise RuntimeError("WeightEMA already swapped — call swap_out() before swap_into() again.")
        for name, p in model.named_parameters():
...
def run_one(
    ...
    # ---- Initialise the weight EMA ----
    # `ema_start_step` defers the EMA start so we don't average over the noisy warmup.
    ema_start_step = int(args.ema_start_fraction * args.iterations)
    weight_ema = WeightEMA(base_model, decay=args.ema_decay)
    log0(f"[EMA] decay={args.ema_decay}  start_step={ema_start_step}/{args.iterations}  "
         f"use_at_export={args.ema_use_at_export}")
    ...
    while True:
        ...
        # EMA shadow update — runs once per optimizer step after the live weights move.
        if step >= ema_start_step:
            weight_ema.update(base_model)
    ...
    # ---- Swap live weights for EMA weights before quantisation ----
    # We deliberately do this BEFORE the fp32 checkpoint save so the saved checkpoint
    # also reflects the EMA-quantised weights (this is the configuration we report).
    if args.ema_use_at_export and weight_ema.num_updates > 0:
        live_val_loss, live_val_bpb = eval_val(
            args, model, rank, world_size, device, grad_accum_steps,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        )
        weight_ema.swap_into(base_model)
        ema_val_loss, ema_val_bpb = eval_val(
            args, model, rank, world_size, device, grad_accum_steps,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        )
        log0(f"[EMA] live_val_bpb={live_val_bpb:.4f}  ema_val_bpb={ema_val_bpb:.4f}  "
             f"Δ(ema-live)={ema_val_bpb - live_val_bpb:+.4f}  ema_updates={weight_ema.num_updates}")
        if master_process:
            wandb.log({
                "live_val_bpb_preexport": live_val_bpb,
                "ema_val_bpb_preexport": ema_val_bpb,
                "ema_minus_live_bpb": ema_val_bpb - live_val_bpb,
                "ema_num_updates": weight_ema.num_updates,
            })
